...
<body class="font-sans bg-[#F7F8FA] text-[#1A1A2E] min-h-screen flex flex-col">
    ...
    <header class="bg-white border-b border-gray-200 sticky top-0 z-[100]">
            ...
            <div class="flex items-center gap-3">
                ...
                @if(auth()->user()->hasRole('Landlord'))
                    <a href="{{ route('landlord.listings.index') }}"
                        class="hidden sm:flex items-center gap-2 h-10 px-5 border border-gray-200 rounded-full bg-white text-[13.5px] font-semibold text-gray-800 hover:shadow-md transition-all">
        ...
        {{-- 2. Search Pill + Category Strip (browse pages only) --}}
        @if($searchBar ?? true)
