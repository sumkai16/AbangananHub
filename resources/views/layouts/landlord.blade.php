...
<body class="font-sans bg-[#F7F8FA] text-[#0F172A] min-h-screen" x-data="{
    ...
    <div class="flex min-h-screen">
        ...
        <aside :class="[sidebarOpen ? 'translate-x-0' : '-translate-x-full', sidebarCollapsed ? 'lg:w-20' : 'lg:w-64']"
            class="fixed inset-y-0 left-0 z-50 w-64 bg-[#0F172A] border-r border-white/10 flex flex-col transition-all duration-300 lg:translate-x-0">

            {{-- Floating collapse toggle (desktop only) --}}
            <button @click="sidebarCollapsed = !sidebarCollapsed"
                class="hidden lg:flex absolute top-[26px] -right-3 w-6 h-6 rounded-full bg-[#3B82F6] text-white items-center justify-center shadow-md hover:brightness-95 transition-all duration-200 z-10"
            ...
            <a href="{{ route('landlord.dashboard') }}"
                ...
                <div class="w-9 h-9 rounded-xl bg-[#3B82F6] flex items-center justify-center shadow-sm shrink-0">
                ...
                <span x-show="!sidebarCollapsed" x-cloak
                    ...
                    Abanganan<span class="text-[#3B82F6]">Hub</span>
                </span>
            </a>
            ...
            <nav class="flex-1 overflow-hidden px-3 py-5">
                ...
                {{-- Dashboard --}}
                <a href="{{ route('landlord.dashboard') }}" :title="sidebarCollapsed ? 'Dashboard' : ''"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 transition-colors duration-200
                          {{ $current === 'landlord.dashboard' ? 'bg-[#3B82F6] text-white font-semibold' : 'text-white/70 hover:bg-white/10' }}">
                ...
                {{-- Properties --}}
                <a href="{{ route('landlord.properties.index') }}" :title="sidebarCollapsed ? 'Properties' : ''"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 transition-colors duration-200
                          {{ str_starts_with($current, 'landlord.properties') && !str_contains($current, 'units') ? 'bg-[#3B82F6] text-white font-semibold' : 'text-white/70 hover:bg-white/10' }}">
                    <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                        class="shrink-0">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Properties</span>
                </a>

                {{-- Units — grayed, no global route yet --}}
                <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 text-white/30 cursor-not-allowed select-none"
                    title="View units from within a specific property for now">
                    <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                        class="shrink-0">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Units</span>
                    <span x-show="!sidebarCollapsed" x-cloak
                ...
                {{-- Reservations --}}
                <a href="{{ route('landlord.reservations.index') }}" :title="sidebarCollapsed ? 'Reservations' : ''"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 transition-colors duration-200
                          {{ str_starts_with($current, 'landlord.reservations') ? 'bg-[#3B82F6] text-white font-semibold' : 'text-white/70 hover:bg-white/10' }}">
                    <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                        class="shrink-0">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                    </svg>
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Reservations</span>
                </a>

                {{-- Tenants — grayed --}}
                <div
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 text-white/30 cursor-not-allowed select-none">
                    <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                        class="shrink-0">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
                    </svg>
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Tenants</span>
                    <span x-show="!sidebarCollapsed" x-cloak
                        class="ml-auto text-[9px] font-bold uppercase tracking-wider bg-white/10 text-white/40 px-1.5 py-0.5 rounded-full whitespace-nowrap">Soon</span>
                </div>

                {{-- Messages --}}
                <a href="{{ route('conversations.index') }}" :title="sidebarCollapsed ? 'Messages' : ''"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 transition-colors duration-200
                          {{ str_starts_with($current, 'conversations') ? 'bg-[#3B82F6] text-white font-semibold' : 'text-white/70 hover:bg-white/10' }}">
                    <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                        class="shrink-0">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 0 1-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Messages</span>
                </a>
                ...
                <div
                    ...
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Occupancy</span>
                    <span x-show="!sidebarCollapsed" x-cloak
                        class="ml-auto text-[9px] font-bold uppercase tracking-wider bg-white/10 text-white/40 px-1.5 py-0.5 rounded-full whitespace-nowrap">Soon</span>
                </div>
                ...
                <div
                    ...
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Reports</span>
                    <span x-show="!sidebarCollapsed" x-cloak
                        class="ml-auto text-[9px] font-bold uppercase tracking-wider bg-white/10 text-white/40 px-1.5 py-0.5 rounded-full whitespace-nowrap">Soon</span>
                </div>
                ...
                <div
                    ...
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Reviews</span>
                    <span x-show="!sidebarCollapsed" x-cloak
                        class="ml-auto text-[9px] font-bold uppercase tracking-wider bg-white/10 text-white/40 px-1.5 py-0.5 rounded-full whitespace-nowrap">Soon</span>
                </div>
                ...
                <a href="{{ route('profile.edit') }}" :title="sidebarCollapsed ? 'Settings' : ''"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-1 transition-colors duration-200
                          {{ $current === 'profile.edit' ? 'bg-[#3B82F6] text-white font-semibold' : 'text-white/70 hover:bg-white/10' }}">
                    <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                        class="shrink-0">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <span x-show="!sidebarCollapsed" x-cloak class="whitespace-nowrap">Settings</span>
                </a>
            </nav>

            {{-- Help footer card --}}
            <div x-show="!sidebarCollapsed" x-cloak class="p-4 border-t border-white/10 shrink-0">
                <div class="bg-white/10 rounded-xl p-3.5">
                    ...
                    <p class="text-[11px] text-white/50 mb-2.5 leading-relaxed">Visit our Help Center or contact
                        support.</p>
                    <a href="{{ route('about') }}"
                        class="block text-center text-[11px] font-semibold bg-[#3B82F6] text-white rounded-lg py-2 hover:brightness-95 transition-all duration-200 whitespace-nowrap">
                        Go to Help Center
                    </a>
                </div>
            </div>
        ...
        <div :class="sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'"
            ...
            <header
                ...
                <button @click="sidebarOpen = !sidebarOpen"
                    class="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-[#F1F5F9] text-[#0F172A] transition-colors duration-200">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('notifications.index') }}"
                        class="relative flex items-center justify-center w-10 h-10 rounded-full border border-[#9B9F98]/20 bg-white text-[#0F172A]/70 hover:bg-[#F1F5F9] transition-colors duration-200">
                        @if($unread > 0)
                            <div
                    ...
                    <div class="relative">
                        <button id="landlord-avatar-btn" aria-expanded="false"
                            class="flex items-center gap-2 pl-1 pr-3 py-1 rounded-full hover:bg-[#F1F5F9] transition-colors duration-200 focus:outline-none">
                            <span
                                class="w-9 h-9 rounded-full bg-[#3B82F6] text-white text-[14px] font-bold flex items-center justify-center shrink-0">
                                {{ strtoupper(substr(auth()->user()->first_name, 0, 1)) }}
                            </span>
                            <span class="hidden sm:flex flex-col items-start leading-tight">
                                <span class="text-[13px] font-semibold text-[#0F172A]">{{ auth()->user()->first_name }}
                        ...
                        <div id="landlord-avatar-menu"
                            ...
                            <a href="{{ route('properties.index') }}"
                                class="flex items-center gap-3 px-4 py-2.5 text-[13.5px] font-medium text-[#0F172A] hover:bg-[#F1F5F9]">
                                Browse as Tenant
                            </a>
