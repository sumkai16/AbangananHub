...
<div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    @else
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-0 bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden shadow-sm min-h-[500px]">
            ...
            <div class="border-r border-[#E2E8F0] flex flex-col divide-y divide-[#E2E8F0] overflow-y-auto max-h-[700px]">
                @foreach($notifications as $n)
                    @php
                        $isSelected = $selected && $selected->notification_id === $n->notification_id;
                        $iconConfig = match(true) {
                            str_contains($n->title, 'Review') || str_contains($n->title, 'Reply') => ['bg' => '#FBBF24', 'icon' => 'M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z'],
                            str_contains($n->title, 'Approved') || str_contains($n->title, 'approved') => ['bg' => '#22C55E', 'icon' => 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                            str_contains($n->title, 'Rejected') || str_contains($n->title, 'rejected') || str_contains($n->title, 'Cancel') => ['bg' => '#EF4444', 'icon' => 'M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                            str_contains($n->title, 'Reservation') || str_contains($n->title, 'reservation') => ['bg' => '#2AA7A1', 'icon' => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5'],
                            default => ['bg' => '#156F8C', 'icon' => 'M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0'],
                        };
                    @endphp
            ...
            {{-- RIGHT: Detail panel --}}
            <div class="bg-[#F7FCFC] p-6 flex flex-col {{ $selected ? '' : 'items-center justify-center' }}">
                @if($selected)
                    {{-- Panel header --}}
