{{--
    ...
    strips fill the section height instead of a fixed size.
           parallax — give each layer its own scroll depth (see resources/js/parallax.js); the parent
    section must carry data-parallax-scope. Nearer layers travel further than far ones.
--}}
@props(['compact' => false, 'parallax' => false])
...
@php
    ...
    // Signed px of travel per layer; empty when the section isn't parallaxed.
    $depth = fn (int $px) => $parallax ? 'data-parallax="' . $px . '"' : '';
@endphp

<div class="pointer-events-none absolute inset-0 -z-10" aria-hidden="true">
    {{-- Soft glows: blurred washes of colour, fine to sit behind content. --}}
    <div {!! $depth(44) !!} class="absolute -left-40 {{ $compact ? '-top-16' : 'top-8' }} {{ $blob }} rounded-full bg-[#FF8A66]/20 blur-3xl"></div>
    <div {!! $depth(-36) !!} class="absolute -right-48 {{ $compact ? 'bottom-[-6rem]' : 'top-[38%]' }} {{ $compact ? 'w-[360px] h-[360px]' : 'w-[560px] h-[560px]' }} rounded-full bg-[#7C8CE0]/20 blur-3xl"></div>
    @unless($compact)
    ...
    <div class="absolute inset-y-0 left-0 w-[max(0px,calc((100%-1400px)/2))] overflow-hidden">
        <div {!! $depth(14) !!} class="absolute left-0 top-0 w-full {{ $compact ? 'h-full' : 'h-[560px]' }} opacity-70 {{ $dots }} {{ $fades['right'] }}"></div>
        <div {!! $depth(22) !!} class="absolute left-0 w-full {{ $compact ? 'bottom-0 h-[70%]' : 'top-[46%] h-[420px]' }} opacity-50 [background-image:repeating-linear-gradient(135deg,rgba(255,138,102,.55)_0_1px,transparent_1px_16px)] {{ $fades['right'] }}"></div>
        <svg {!! $depth(30) !!} class="absolute left-0 w-full {{ $compact ? 'top-[8%] h-[45%]' : 'top-[18%] h-[200px]' }} text-[#FF8A66]" viewBox="0 0 420 200" fill="none" preserveAspectRatio="none">
    ...
    <div class="absolute inset-y-0 right-0 w-[max(0px,calc((100%-1400px)/2))] overflow-hidden">
        <div {!! $depth(16) !!} class="absolute right-0 bottom-0 w-full {{ $compact ? 'h-full' : 'h-[520px]' }} opacity-60 {{ $dots }} {{ $fades['left'] }}"></div>
        <div {!! $depth(24) !!} class="absolute right-0 top-0 w-full {{ $compact ? 'h-[70%]' : 'h-[360px]' }} opacity-40 [background-image:repeating-linear-gradient(135deg,rgba(6,13,38,.35)_0_1px,transparent_1px_16px)] {{ $fades['left'] }}"></div>
        <svg {!! $depth(34) !!} class="absolute right-0 w-full {{ $compact ? 'bottom-[8%] h-[45%]' : 'top-[62%] h-[200px]' }} text-[#5B6A8E]" viewBox="0 0 420 200" fill="none" preserveAspectRatio="none">
