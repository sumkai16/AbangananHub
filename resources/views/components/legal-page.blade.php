        ...
        <div class="mt-10 grid lg:grid-cols-[240px_1fr] gap-8 lg:gap-12 items-start">
                    ...
                    @foreach($sections as $i => $section)
                        <li>
                            <a href="#section-{{ $i + 1 }}"
                                ...
                                <span class="{{ $font }} font-bold text-[#B35A3D] w-5 shrink-0">{{ $i + 1 }}.</span>
            ...
            <div class="space-y-4 min-w-0">
                @foreach($sections as $i => $section)
                    <section id="section-{{ $i + 1 }}" class="scroll-mt-[88px] bg-white border border-[#E2E4EC] rounded-2xl p-5 sm:p-7">
                        <h2 class="flex items-center gap-3 {{ $font }} text-[18px] sm:text-[20px] font-extrabold tracking-tight text-[#060D26]">
                            <span class="inline-flex items-center justify-center min-w-8 h-8 px-2 rounded-full border border-[#FF8A66]/60 bg-[#FF8A66]/10 text-[12px] font-bold text-[#060D26]">{{ $i + 1 }}</span>
                            {{ $section['title'] }}
                        </h2>
                                    ...
                                    <ul class="space-y-1.5">
                                        @foreach($block['list'] as $item)
                                            <li class="flex gap-2.5">
                                                <span class="mt-2 w-1.5 h-1.5 rounded-full bg-[#FF8A66] shrink-0" aria-hidden="true"></span>
                                                <span>{{ $item }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#FF8A66]/60 bg-gradient-to-br from-[#FF8A66]/15 via-[#FF8A66]/5 to-white p-5">
                    ...
                    <a href="{{ route('home') }}"
                        class="inline-flex items-center gap-2 {{ $font }} text-[13px] font-bold text-[#060D26] bg-[#FF8A66] hover:bg-[#E96F4F] px-5 py-2.5 rounded-full shadow-[0_8px_20px_rgba(255,138,102,0.35)] transition-colors">
                        Back to listings
