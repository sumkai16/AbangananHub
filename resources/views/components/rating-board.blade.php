...
<div class="bg-white border border-[#E2E4EC] rounded-2xl shadow-[0_1px_3px_rgba(6,13,38,0.06)] overflow-hidden">
    <div class="px-5 py-4 border-b border-[#E2E4EC]">
        <h2 class="text-[15px] font-normal text-[#060D26]">{{ $title }}</h2>
        @if($sub)
            <p class="text-[12px] text-[#5B6A8E] mt-0.5">{{ $sub }}</p>
        @endif
    </div>
    ...
    @else
        <ul class="divide-y divide-[#E2E4EC]">
            @foreach($rows as $i => $row)
                ...
                @endphp
                <li>
                    <a href="{{ $href }}" class="flex items-center gap-3 px-5 py-3 hover:bg-[#F7F8FC] transition-colors">
                        <span class="w-6 text-[13px] font-bold text-[#94A3B8] shrink-0">{{ $i + 1 }}</span>

                        @if($isUser)
                            <span class="w-9 h-9 rounded-full bg-[#ECEEF6] flex items-center justify-center text-[12px] font-bold text-[#060D26] shrink-0">
                                {{ $initials ?: '?' }}
                            </span>
                        @else
                            <span class="w-9 h-9 rounded-lg bg-[#ECEEF6] flex items-center justify-center shrink-0">
                        ...
                        <span class="min-w-0 flex-1">
                            <span class="block text-[13.5px] font-semibold text-[#060D26] truncate">{{ $name }}</span>
                            <span class="block text-[11.5px] text-[#5B6A8E]">{{ $row['count'] }} {{ \Illuminate\Support\Str::plural('rating', $row['count']) }}</span>
                        </span>
