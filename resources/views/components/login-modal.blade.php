    ...
    <div class="relative p-8">
        ...
        <div class="mb-6">
            <div class="flex items-center gap-2 mb-4">
                ...
                <span class="text-[#156F8C] font-bold text-lg tracking-tight">Abanganan<span class="text-[#156F8C]">Hub</span></span>
            </div>
            <h2 class="text-2xl font-black text-[#156F8C] tracking-tight">Welcome Back!</h2>
            <p class="text-[#64748B] text-sm font-medium mt-0.5">Login to continue to AbangananHub</p>
        </div>
        ...
        @if ($errors->any() && !request()->routeIs('register'))
            <div class="mb-4 flex items-start gap-3 bg-[#EF4444]/10 border border-[#EF4444]/30 rounded-lg px-4 py-3">
                ...
                <p class="text-sm text-[#EF4444] font-semibold">{{ $errors->first() }}</p>
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            {{-- Email --}}
            <div>
                <label for="login_email" class="block text-sm font-bold text-[#156F8C] mb-1">Email Address</label>
                <input id="login_email" type="email" name="email" value="{{ old('email') }}" required autocomplete="username" placeholder="Enter your email"
                    class="w-full px-3 py-2.5 bg-slate-50/50 border border-slate-300 rounded-lg text-sm text-[#156F8C] placeholder-[#64748B] transition-all focus:bg-white focus:outline-none focus:border-[#2AA7A1] focus:ring-4 focus:ring-[#2AA7A1]/10" />
            </div>

            {{-- Password --}}
            <div x-data="{ showPassword: false }">
                <label for="login_password" class="block text-sm font-bold text-[#156F8C] mb-1">Password</label>
                <div class="relative">
            ...
            <div class="flex items-center justify-between pt-1">
                <label class="flex items-center gap-2 text-sm text-[#5E6968] font-semibold cursor-pointer select-none">
                    <input type="checkbox" name="remember" class="rounded border-slate-300 text-[#2AA7A1] focus:ring-[#2AA7A1] transition-colors w-4 h-4" />
                    Remember me
                </label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-sm text-[#156F8C] hover:text-[#156F8C] font-bold transition-colors hover:underline">
                        Forgot Password?
                    </a>
                @endif
            </div>
        ...
        <p class="text-center text-sm text-[#64748B] font-medium mt-6">
            Don't have an account?
            <button x-on:click="$dispatch('close'); setTimeout(() => $dispatch('open-modal', 'register-modal'), 300)" class="text-[#156F8C] font-bold hover:underline ml-1 focus:outline-none">Register here</button>
        </p>
    </div>
</x-modal>
