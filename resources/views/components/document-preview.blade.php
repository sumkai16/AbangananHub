...
<div class="rounded-xl overflow-hidden border border-[#E2E4EC] bg-[#F8FAFC] relative" x-data="{ expanded: false, failed: false }">
    <button type="button" @click="expanded = true" x-show="!failed"
            class="absolute top-2 right-2 z-10 inline-flex items-center gap-1 h-7 px-2.5 rounded-full bg-white/95 border border-[#E2E4EC] text-[11px] font-medium text-[#060D26] shadow-sm hover:bg-white transition-colors">
    ...
    <div x-show="expanded" x-cloak
        ...
        <div class="bg-white rounded-2xl overflow-hidden w-full h-full max-w-5xl relative flex flex-col shadow-2xl">
            <button type="button" @click="expanded = false"
                    class="absolute top-3 right-3 z-10 w-9 h-9 rounded-full bg-white border border-[#E2E4EC] flex items-center justify-center shadow-sm hover:bg-[#F1F5F9] transition-colors">
