...
{{--
    ...
    a labelled Search button. `variant="header"` keeps the original
    icon-less, icon-only button. Both are the standard navy CTA fill —
    no more color split between them now that CTA and primary fill are
    the same navy token (Sept 2026 navy/gold identity).
--}}
...
<form action="{{ route('properties.index') }}" method="GET"
    ...
    <div
        ...
        <div class="flex flex-col justify-center min-w-0 flex-1">
            <label for="search-where-{{ $variant }}"
                class="{{ $labelSize }} font-bold text-[#5B6A8E] tracking-wide uppercase truncate cursor-pointer">Where</label>
            <input type="text" name="location" id="search-where-{{ $variant }}" value="{{ request('location') }}"
                placeholder="Barangay, area, or landmark"
                class="p-0 border-none bg-transparent {{ $inputSize }} text-[#060D26] focus:ring-0 placeholder-[#94A3B8] w-full outline-none mt-0.5 truncate">
        </div>
    </div>

    <div
        class="flex-1 flex items-center gap-2.5 {{ $pad }} border-r border-[#E2E4EC] hover:bg-[#F7F8FC] transition-colors w-[33%]">
        @if($isHero)
            <svg class="{{ $icon }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"
                aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex flex-col justify-center min-w-0 flex-1">
            @if($isHero)
                ...
                <label id="search-type-label-{{ $variant }}"
                    class="{{ $labelSize }} font-bold text-[#5B6A8E] tracking-wide uppercase truncate cursor-pointer">Type</label>
                <x-styled-select name="type" :options="['' => 'Any type', 'Bedspace' => 'Bedspace', 'Room' => 'Room', 'Apartment' => 'Apartment', 'House' => 'House']"
                    ...
                    class="p-0 border-none bg-transparent {{ $inputSize }} text-[#060D26] w-full mt-0.5" />
            @else
                <label for="search-type-{{ $variant }}"
                    class="{{ $labelSize }} font-bold text-[#5B6A8E] tracking-wide uppercase truncate cursor-pointer">Type</label>
                <select name="type" id="search-type-{{ $variant }}"
                    class="p-0 border-none bg-transparent {{ $inputSize }} text-[#060D26] focus:ring-0 w-full outline-none appearance-none cursor-pointer mt-0.5 truncate">
    ...
    <div
        class="flex-1 flex items-center justify-between {{ $isHero ? 'pl-3 pr-2 sm:pl-6 sm:pr-2 py-2 sm:py-2.5' : 'pl-3 pr-2 sm:pl-7 sm:pr-2 py-2' }} hover:bg-[#F7F8FC] rounded-r-full transition-colors w-[33%] overflow-hidden">
