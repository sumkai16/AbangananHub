@php $toolFilterCount = \App\Support\BrowseFilters::activeCount(request()); @endphp
<div x-data="{ mapVisible: false }" @browse-map-state.window="mapVisible = $event.detail" class="flex items-center gap-2">
    <button type="button" @click="$dispatch('browse-toggle-map')"
        class="inline-flex items-center gap-1.5 h-9 px-3.5 rounded-full border border-[#E2E4EC] bg-white text-[#060D26] font-jakarta text-[12.5px] font-bold shadow-sm hover:border-[#FF8A66] transition-colors cursor-pointer">
