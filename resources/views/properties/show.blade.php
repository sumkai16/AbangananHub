    ...
    <div class="max-w-[1200px] mx-auto px-4 sm:px-6 py-8 relative">
        ...
        <div class="mb-6 relative z-10">
            <a href="{{ route('properties.index') }}"
                class="inline-flex items-center gap-2 text-sm font-semibold text-[#9B9F98] hover:text-[#2A2523] transition-colors group">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative z-10">
            ...
            <div class="lg:col-span-7 xl:col-span-8 space-y-8">
                ...
                @if($property->media->count() >= 5)
                    <div class="relative grid grid-cols-4 gap-2 aspect-[2/1] rounded-3xl overflow-hidden shadow-sm border border-[#D7E8F3] group cursor-pointer" onclick="openLightbox(0)">
                        ...
                        <button class="absolute bottom-4 right-4 bg-white/90 backdrop-blur-md text-[#2A2523] text-xs font-bold px-4 py-2.5 rounded-xl shadow-sm hover:bg-white transition-all flex items-center gap-1.5 border border-white/20">
                ...
                @elseif($property->media->count() > 0)
                    <div class="relative rounded-3xl overflow-hidden bg-[#F0EDE8] aspect-[16/9] shadow-sm border border-[#D7E8F3] cursor-pointer group" onclick="openLightbox(0)">
                ...
                @else
                    <div class="rounded-3xl bg-[#F0EDE8] aspect-[16/9] border border-dashed border-[#9B9F98] flex flex-col items-center justify-center text-[#9B9F98] shadow-sm">
                ...
                <div>
                    <div class="flex flex-wrap items-center gap-2 mb-3">
                        <span class="inline-flex items-center text-xs font-bold px-2.5 py-1 rounded-lg bg-[#D7E8F3] text-[#2A2523] border border-[#61B2F0]/20 shadow-sm">
                    ...
                    <h1 class="text-2xl sm:text-3xl font-black text-[#2A2523] tracking-tight mb-3">
                        {{ $property->title }}
                    </h1>
                ...
                <div class="pt-6 border-t border-[#D7E8F3]">
                    <h2 class="text-base font-bold text-[#2A2523] mb-2">About this place</h2>
                    <p class="text-sm text-[#2A2523]/80 leading-relaxed whitespace-pre-line">{{ $property->description }}</p>
                </div>

                {{-- AMENITIES --}}
                @if($property->amenities->count() > 0)
                    <div class="pt-6 border-t border-[#D7E8F3]">
                        <h2 class="text-base font-bold text-[#2A2523] mb-4">Amenities</h2>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            @foreach($property->amenities as $amenity)
                                <div class="flex items-center gap-3 text-sm text-[#2A2523] font-medium">
                                    <div class="w-8 h-8 rounded-lg bg-[#D7E8F3] border border-[#61B2F0]/20 flex items-center justify-center flex-shrink-0">
                                        <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#61B2F0" stroke-width="2.5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            @foreach($approvedUnits as $unit)
                                ...
                                <div class="rounded-2xl border {{ $isAvailable ? 'border-[#D7E8F3] bg-white' : 'border-[#F0EDE8] bg-[#F0EDE8]/50' }} p-4 flex flex-col justify-between">
                                    ...
                                    {{-- Action --}}
                                    @if(!$isAvailable)
                                        <div class="w-full text-center bg-[#F0EDE8] text-[#9B9F98] text-xs font-bold py-2.5 rounded-xl cursor-not-allowed">
                                            Unavailable
                                        </div>
                                    @elseif($isOwner)
                                        <div class="w-full text-center bg-[#F0EDE8] text-[#9B9F98] text-xs font-bold py-2.5 rounded-xl cursor-not-allowed">
                                    ...
                                    @elseif(!auth()->check())
                                        <button type="button" x-data x-on:click="$dispatch('open-modal', 'login-modal')"
                                            class="w-full py-2.5 rounded-xl bg-[#61B2F0] hover:brightness-95 text-white text-xs font-bold transition-all">
                                            Log in to inquire
                                        </button>
                                    @elseif($hasActiveReservation)
                                        <div class="w-full text-center bg-[#D7E8F3] text-[#2A2523] text-xs font-bold py-2.5 rounded-xl cursor-not-allowed">
                                    ...
                                    @else
                                        <form action="{{ route('reservations.store') }}" method="POST">
                                            ...
                                            <button type="submit"
                                                class="w-full py-2.5 rounded-xl bg-[#61B2F0] hover:brightness-95 text-white text-xs font-bold shadow-sm transition-all">
                                                Inquire
                                            </button>
                                        </form>
                                    @endif
                ...
                <div class="pt-6 border-t border-[#D7E8F3]">
                    ...
                    @forelse($property->reviews as $review)
                        <div class="mb-6 last:mb-0 bg-[#F0EDE8]/40 border border-[#D7E8F3] p-4 rounded-2xl">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-xl bg-[#61B2F0] text-white text-xs font-black flex items-center justify-center flex-shrink-0 shadow-sm">
                            ...
                            <p class="text-sm text-[#2A2523]/80 leading-relaxed pl-12">
                ...
                <div class="pt-6 border-t border-[#D7E8F3]">
                    ...
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 text-xs font-semibold text-[#9B9F98]">
                        ...
                        <span class="flex items-center gap-1.5">
                            <span class="w-2.5 h-2.5 rounded-full bg-[#D7E8F3] border border-[#9B9F98] flex-shrink-0"></span>
                            Transport
                        </span>
                    </div>
            ...
            <div class="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-6 space-y-4 w-full">
                ...
                <div class="bg-white rounded-3xl border border-[#D7E8F3] shadow-sm p-6">
                    <div class="mb-4 pb-4 border-b border-[#D7E8F3]">
                        <div class="flex items-baseline gap-1">
                            <span class="text-xs font-bold text-[#9B9F98] uppercase tracking-wider">Starting at</span>
                        </div>
                        <div class="flex items-baseline gap-1 mt-1">
                            <span class="text-3xl font-black text-[#2A2523] tracking-tight">
                    ...
                    @auth
                        @if((int) auth()->id() !== (int) $property->landlord_id)
                            <form action="{{ route('conversations.store') }}" method="POST">
                                ...
                                <button type="submit"
                                    class="w-full py-3 px-4 rounded-xl border border-[#D7E8F3] bg-white text-[#2A2523] hover:bg-[#F0EDE8] text-sm font-bold shadow-sm transition-all duration-200 flex items-center justify-center gap-1.5">
                        ...
                        @else
                            <div class="w-full py-3 px-4 text-center rounded-xl border border-[#D7E8F3] bg-[#F0EDE8] text-[#9B9F98] text-sm font-bold cursor-not-allowed">
                ...
                <div class="bg-white border border-[#D7E8F3] rounded-2xl p-5 shadow-sm">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-11 h-11 rounded-xl bg-[#61B2F0] text-white text-base font-black flex items-center justify-center flex-shrink-0 shadow-sm">
