...
<div class="space-y-8">
    ...
    {{-- Property Type --}}
    <div>
        <label class="block text-sm font-bold text-[#1F2937] mb-3">Property Type</label>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            @foreach($types as $type => $svgPath)
                <label class="relative cursor-pointer">
                    ...
                    <div
                        class="flex flex-col items-center gap-2 py-4 px-2 rounded-xl border-2 border-gray-200 text-gray-500 peer-checked:border-[#2AA7A1] peer-checked:bg-[#EEF8F8] peer-checked:text-[#1F2937] transition-colors">
        ...
        @error('property_type')<p class="mt-2 text-sm text-[#EF4444]">{{ $message }}</p>@enderror
    </div>

    {{-- Title --}}
    <div>
        <label for="title" class="block text-sm font-bold text-[#1F2937] mb-2">Listing Title</label>
        <input type="text" id="title" name="title" value="{{ old('title', $property->title ?? '') }}"
            placeholder="e.g. Cozy 2BR Apartment near IT Park"
            class="w-full h-12 px-4 rounded-xl border border-gray-200 focus:border-[#2AA7A1] focus:ring-2 focus:ring-[#2AA7A1]/30 outline-none text-sm">
        @error('title')<p class="mt-2 text-sm text-[#EF4444]">{{ $message }}</p>@enderror
    </div>

    {{-- Description --}}
    <div>
        <label for="description" class="block text-sm font-bold text-[#1F2937] mb-2">Description</label>
        <textarea id="description" name="description" rows="5"
            placeholder="Describe the space, what's included, house rules, nearby landmarks..."
            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#2AA7A1] focus:ring-2 focus:ring-[#2AA7A1]/30 outline-none text-sm">{{ old('description', $property->description ?? '') }}</textarea>
        @error('description')<p class="mt-2 text-sm text-[#EF4444]">{{ $message }}</p>@enderror
    </div>

    {{-- Address --}}
    <div>
        <label for="address" class="block text-sm font-bold text-[#1F2937] mb-2">Address</label>
    ...
    {{-- Coordinates --}}
    <div>
        <label class="block text-sm font-bold text-[#1F2937] mb-2">Coordinates</label>
        <p class="text-xs text-[#64748B] mb-3">
            Map picker isn't built yet — for now, open
            <a href="https://www.google.com/maps" target="_blank"
                class="text-[#156F8C] font-semibold hover:underline">Google Maps</a>,
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <input type="number" step="0.0000001" name="latitude" id="latitude"
                    ...
                    class="w-full h-12 px-4 rounded-xl border border-gray-200 focus:border-[#2AA7A1] focus:ring-2 focus:ring-[#2AA7A1]/30 outline-none text-sm">
                <span class="text-xs text-[#64748B] mt-1 block">Latitude</span>
                @error('latitude')<p class="mt-1 text-sm text-[#EF4444]">{{ $message }}</p>@enderror
            </div>
            <div>
                <input type="number" step="0.0000001" name="longitude" id="longitude"
                    value="{{ old('longitude', $property->longitude ?? '') }}" placeholder="123.8854"
                    class="w-full h-12 px-4 rounded-xl border border-gray-200 focus:border-[#2AA7A1] focus:ring-2 focus:ring-[#2AA7A1]/30 outline-none text-sm">
                <span class="text-xs text-[#64748B] mt-1 block">Longitude</span>
                @error('longitude')<p class="mt-1 text-sm text-[#EF4444]">{{ $message }}</p>@enderror
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            <label for="rental_fee" class="block text-sm font-bold text-[#1F2937] mb-2">Monthly Rent</label>
            <div class="relative">
                ...
                <input type="number" step="0.01" id="rental_fee" name="rental_fee"
                    ...
                    class="w-full h-12 pl-8 pr-4 rounded-xl border border-gray-200 focus:border-[#2AA7A1] focus:ring-2 focus:ring-[#2AA7A1]/30 outline-none text-sm">
            </div>
            @error('rental_fee')<p class="mt-2 text-sm text-[#EF4444]">{{ $message }}</p>@enderror
        </div>
        <div>
            <label for="occupancy_limit" class="block text-sm font-bold text-[#1F2937] mb-2">Max Occupants</label>
            <input type="number" id="occupancy_limit" name="occupancy_limit"
                ...
                class="w-full h-12 px-4 rounded-xl border border-gray-200 focus:border-[#2AA7A1] focus:ring-2 focus:ring-[#2AA7A1]/30 outline-none text-sm">
            @error('occupancy_limit')<p class="mt-2 text-sm text-[#EF4444]">{{ $message }}</p>@enderror
        </div>
    </div>
    ...
    @if($isEdit && $property->media->count() > 0)
        <div>
            <label class="block text-sm font-bold text-[#1F2937] mb-2">Current Photos</label>
            <p class="text-xs text-[#64748B] mb-3">Removing individual photos isn't supported yet — this is add-only for
                now.</p>
    ...
    {{-- Photo upload --}}
    <div>
        <label class="block text-sm font-bold text-[#1F2937] mb-2">{{ $isEdit ? 'Add More Photos' : 'Photos' }}</label>
        <label for="photos"
            class="flex flex-col items-center justify-center gap-2 h-32 rounded-xl border-2 border-dashed border-gray-300 hover:border-[#2AA7A1] cursor-pointer transition-colors">
