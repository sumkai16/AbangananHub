...
<div x-data="messageNotifications()" x-cloak>
    ...
    <div style="position: fixed; bottom: 80px; right: 20px; z-index: 9998; display: flex; flex-direction: column-reverse; gap: 8px; pointer-events: none;">
        <template x-for="toast in toasts" :key="toast.id">
            <div @click="openConversation(toast.conversation_id)"
                ...
                class="pointer-events-auto cursor-pointer w-[340px] bg-white border border-[#9B9F98]/20 rounded-2xl p-3.5 shadow-[0_8px_24px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.16)] transition-shadow duration-200">
                <div class="flex gap-3 items-start">
                    <div class="w-10 h-10 rounded-full bg-[#2A2523] flex items-center justify-center shrink-0">
                        <span class="text-white text-[14px] font-bold" x-text="toast.sender_initial"></span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between gap-2 mb-0.5">
                            <span class="text-[13px] font-bold text-[#2A2523] truncate" x-text="toast.sender_name"></span>
                            <span class="text-[11px] text-[#9B9F98] shrink-0">now</span>
                        </div>
                        <div class="text-[11px] text-[#9B9F98] truncate mb-1" x-show="toast.property_title">
                            <span x-text="toast.property_title"></span>
                            <template x-if="toast.unit_label">
                                <span> · <span x-text="toast.unit_label"></span></span>
                            </template>
                        </div>
                        <div class="text-[13px] text-[#2A2523]/80 truncate" x-text="toast.message"></div>
                    </div>
                    <button @click.stop="dismissToast(toast.id)"
                        class="shrink-0 w-6 h-6 flex items-center justify-center rounded-full hover:bg-[#F0EDE8] text-[#9B9F98] transition-colors">
    ...
    <div x-show="panelOpen"
        ...
        class="fixed bottom-5 right-5 z-[9997] w-[360px] bg-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.15)] border border-[#9B9F98]/15 overflow-hidden"
        @click.away="closeBubblePanel()">
        <div x-ref="bubblePanelBody">
            <div class="px-4 py-8 text-center">
                <div class="w-6 h-6 border-2 border-[#9B9F98] border-t-transparent rounded-full animate-spin mx-auto"></div>
            </div>
        </div>
    </div>
    ...
    <button x-show="!panelOpen" @click="openBubblePanel()"
        class="fixed bottom-5 right-5 z-[9997] w-[52px] h-[52px] rounded-full bg-[#61B2F0] flex items-center justify-center shadow-[0_4px_16px_rgba(97,178,240,0.35)] hover:brightness-95 transition-all duration-200 focus:outline-none"
        x-transition:enter="transition ease-out duration-200"
        ...
        <div x-show="unreadCount > 0"
            class="absolute -top-0.5 -right-0.5 min-w-[20px] h-[20px] rounded-full bg-[#BD5434] text-white text-[11px] font-bold flex items-center justify-center px-1 border-2 border-white"
        ...
        return {
            ...
            async fetchBubblePanel() {
                try {
                    ...
                    this.$refs.bubblePanelBody.innerHTML = '<div class="px-4 py-6 text-center text-[13px] text-[#9B9F98]">Failed to load messages.</div>';
                }
            }
        }
    }
