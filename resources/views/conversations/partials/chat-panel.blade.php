...
<div class="flex flex-col h-full" id="chat-panel-root" data-conversation-id="{{ $conversation->conversation_id }}"
    ...
    {{-- Chat header --}}
    <div class="px-5 py-3.5 border-b border-[#E2E4EC] flex items-center justify-between gap-3 flex-shrink-0">
        <div class="flex items-center gap-3 min-w-0">
            <div class="w-9 h-9 rounded-full bg-[#060D26] text-white flex items-center justify-center font-bold text-xs flex-shrink-0">
                {{ strtoupper(substr($otherParty->first_name, 0, 1)) }}
            </div>
            <div class="min-w-0">
                <h2 class="text-[13px] font-normal text-[#060D26] truncate">
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
            ...
            <button type="button" @click="detailsOpen = !detailsOpen"
                ...
                class="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-lg border border-[#E2E4EC] text-[11px] font-bold text-[#5B6A8E] hover:text-[#060D26] hover:bg-[#F7F8FC] cursor-pointer transition-all duration-200">
                Details
    ...
    @if($reservation)
        @if($isTerminal)
            <div class="px-5 py-3 border-b border-[#E2E4EC] bg-[#EF4444]/[0.04] flex items-center gap-2.5 flex-shrink-0">
                <span class="text-[11px] font-bold px-2 py-0.5 rounded-full bg-[#E2E4EC] text-[#EF4444]">{{ $rentalStatus }}</span>
                <p class="text-[11.5px] text-[#5B6A8E] truncate">
        ...
        @else
            ...
            <div class="px-5 py-3 border-b border-[#E2E4EC] bg-[#F7F8FC] flex-shrink-0 flex items-center gap-4 flex-wrap">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    @endif
                    <div class="min-w-0">
                        <p class="text-[12px] font-bold text-[#060D26] truncate">{{ $conversation->property->title }}</p>
                        @if($stripUnit)
                            <p class="text-[11px] text-[#5B6A8E] truncate">{{ $stripUnit->unit_label }}</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex items-center gap-5 ml-auto">
                    <div>
                        ...
                        <p class="text-[12.5px] font-bold text-[#060D26]">&#8369;{{ number_format($stripRent) }}</p>
                    </div>
                    @if($stripDeposit > 0)
                        <div>
                            <p class="text-[9.5px] font-bold text-[#94A3B8] uppercase tracking-wider">Security deposit</p>
                            <p class="text-[12.5px] font-bold text-[#060D26]">&#8369;{{ number_format($stripDeposit) }}</p>
                        </div>
                    @endif
                    <div>
            ...
            <div class="px-5 pt-3.5 pb-3 border-b border-[#E2E4EC] bg-[#F7F8FC] flex-shrink-0">
    ...
    @if($reservation && !$isTerminal)
        ...
        <div class="px-5 py-3 border-b border-[#E2E4EC] flex-shrink-0 max-h-[55%] overflow-y-auto {{ $waitingOnMe ? 'bg-[#ECEEF6]/50' : 'bg-white' }}"
            ...
            @if($isLandlord)
                @if($rentalStatus === 'Inquiry')
                    <div class="flex items-center gap-2">
                        <form action="{{ route('landlord.reservations.advanceNegotiation', $reservation) }}" method="POST" class="flex-1">
                            @csrf @method('PATCH')
                            <button type="submit" class="w-full bg-[#060D26] hover:brightness-95 text-white text-[12px] font-bold py-2 rounded-xl cursor-pointer transition-all duration-200 flex items-center justify-center gap-1.5">
                ...
                @elseif($rentalStatus === 'Under Negotiation')
                    <div class="flex items-center gap-2">
                        <button type="button" @click="showTc = !showTc" class="flex-1 bg-[#060D26] hover:brightness-95 text-white text-[12px] font-bold py-2 rounded-xl cursor-pointer transition-all duration-200 flex items-center justify-center gap-1.5">
                            <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg>
                            Send agreement
                        </button>
                        <button type="button" @click="showReject = !showReject"
                    ...
                    <div x-show="showTc" x-transition x-cloak class="mt-2">
                        <form action="{{ route('landlord.reservations.advanceAgreement', $reservation) }}" method="POST">
                            ...
                            <div class="p-3 bg-[#ECEEF6] rounded-xl border border-[#C9A84C]/20">
                                ...
                                <button type="submit"
                                    class="w-full bg-[#060D26] hover:brightness-95 text-white text-[12px] font-bold py-2 rounded-lg cursor-pointer transition-all duration-200">
                ...
                @elseif($rentalStatus === 'Pending Rental Agreement')
                    <p class="text-[12px] text-[#5B6A8E] font-medium text-center py-1">Waiting for {{ $otherParty->first_name }} to sign the agreement</p>
                ...
                @elseif($rentalStatus === 'Rental Agreement Signed')
                    ...
                    @elseif($pendingPayment)
                        <p class="text-[12px] text-[#5B6A8E] font-medium text-center py-1">{{ $otherParty->first_name }}'s payment is processing</p>
                    @else
                        <p class="text-[12px] text-[#5B6A8E] font-medium text-center py-1">Agreement signed — waiting for payment</p>
                    @endif
            ...
            @if($isTenant)
                @if($rentalStatus === 'Inquiry')
                    <div class="flex items-center gap-2">
                        <p class="flex-1 text-[12px] text-[#5B6A8E] font-medium text-center py-1">Waiting for the landlord to respond</p>
                ...
                @elseif($rentalStatus === 'Under Negotiation')
                    <div class="flex items-center gap-2">
                        <p class="flex-1 text-[12px] text-[#5B6A8E] font-medium text-center py-1">Discuss the terms with the landlord</p>
                        <button type="button" @click="showCancel = !showCancel"
                            class="px-3 py-2 border border-[#EF4444] text-[#EF4444] text-[12px] font-semibold rounded-xl hover:bg-[#EF4444]/5 cursor-pointer transition-all duration-200">
                            Cancel
                        </button>
                ...
                @elseif($rentalStatus === 'Pending Rental Agreement')
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ route('agreements.show', $reservation) }}" target="_blank" rel="noopener"
                            class="flex-1 bg-[#060D26] hover:brightness-95 text-white text-[12px] font-bold py-2 rounded-xl cursor-pointer transition-all duration-200 flex items-center justify-center gap-1.5">
                ...
                @elseif($rentalStatus === 'Rental Agreement Signed')
                    @if($heldPayment)
                        ...
                        <div class="flex items-center gap-2 mt-2">
                            <div class="flex-1 min-w-0">
                                <p class="text-[12px] font-bold text-[#060D26]">Payment held by AbangananHub</p>
                                <p class="text-[11px] text-[#5B6A8E]">Confirm move-in to release &#8369;{{ number_format($heldPayment->amount, 2) }} to the landlord.</p>
                            </div>
                            <a href="{{ route('agreements.show', $reservation) }}" target="_blank" rel="noopener"
                                class="shrink-0 bg-[#060D26] hover:brightness-95 text-white text-[12px] font-bold px-4 py-2 rounded-xl cursor-pointer transition-all duration-200 flex items-center gap-1.5">
                                Confirm move-in
                                <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" class="opacity-60">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
                                </svg>
                    ...
                    @else
                        <a href="{{ route('agreements.show', $reservation) }}" target="_blank" rel="noopener"
                            class="w-full bg-[#060D26] hover:brightness-95 text-white text-[12px] font-bold py-2 rounded-xl cursor-pointer transition-all duration-200 flex items-center justify-center gap-1.5">
    ...
    <div x-show="detailsOpen" x-cloak
        ...
        class="border-b border-[#E2E4EC] bg-[#F7F8FC] overflow-hidden flex-shrink-0 motion-reduce:transform-none">
        ...
        <div class="px-5 py-4">
                ...
                <div class="flex flex-wrap items-center gap-4 px-1">
                    @if($reservation->target_move_in_date)
                        <div class="flex items-center gap-1.5 text-[11px] text-[#5B6A8E]">
                            ...
                            <span>Move in: <span class="text-[#060D26] font-semibold">{{ $reservation->target_move_in_date->format('M j, Y') }}</span></span>
                        </div>
                    @endif
                    @if($reservation->target_move_out_date)
                        <div class="flex items-center gap-1.5 text-[11px] text-[#5B6A8E]">
                            ...
                            <span>Move out: <span class="text-[#060D26] font-semibold">{{ $reservation->target_move_out_date->format('M j, Y') }}</span></span>
                        </div>
                    @endif
                </div>
            @endif
            ...
            @if($reservation?->remarks)
                <div class="mt-3 px-1">
                    <p class="text-[10px] font-bold text-[#5B6A8E] uppercase tracking-wider mb-1">Original message</p>
                    <p class="text-[12px] text-[#060D26] leading-relaxed whitespace-pre-line">{{ $reservation->remarks }}</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div id="message-list" class="flex-1 px-5 py-4 overflow-y-auto flex flex-col gap-1 scroll-smooth bg-[#F7F8FC]"
        ...
        @foreach ($msgs as $i => $message)
            @if($message->is_inquiry_summary)
                ...
                <div class="flex items-end gap-2 self-start max-w-[85%] mb-1.5"
                    ...
                    @else
                        <div class="w-7 h-7 rounded-full bg-[#060D26] text-white flex items-center justify-center text-[11px] font-bold shrink-0 mb-0.5"
                            aria-hidden="true">
                        ...
                        <div class="p-3 space-y-2 text-[12px]">
                            @if($reservation?->target_move_in_date || $reservation?->target_move_out_date)
                                <div class="flex items-center justify-between text-[#5B6A8E]">
                                    <span>Move-in</span>
                                    <span class="font-semibold text-[#060D26]">{{ $reservation->target_move_in_date?->format('M j, Y') ?? '—' }}</span>
                                </div>
                                <div class="flex items-center justify-between text-[#5B6A8E]">
                                    <span>Move-out</span>
                                    <span class="font-semibold text-[#060D26]">{{ $reservation->target_move_out_date?->format('M j, Y') ?? 'Open-ended' }}</span>
                                </div>
                            @endif

                            <div class="pt-2 border-t border-[#E2E4EC] space-y-1">
                                <div class="flex items-center justify-between text-[#5B6A8E]">
                                    <span>Monthly rent</span>
                                    <span class="font-semibold text-[#060D26]">&#8369;{{ number_format($summaryRent) }}</span>
                                </div>
                                @if($summaryDeposit > 0)
                                    <div class="flex items-center justify-between text-[#5B6A8E]">
                                        <span>Security deposit</span>
                                        <span class="font-semibold text-[#060D26]">&#8369;{{ number_format($summaryDeposit) }}</span>
                                    </div>
                                @endif
                                <div class="flex items-center justify-between pt-1 border-t border-[#E2E4EC]">
                                    <span class="font-bold text-[#060D26]">Due at move-in</span>
                                    <span class="font-bold text-[#060D26]">&#8369;{{ number_format($summaryDue) }}</span>
                                </div>
                            </div>

                            @if(trim($message->message ?? '') !== '')
                                <div class="pt-2 border-t border-[#E2E4EC]">
                                    <p class="text-[11px] text-[#5B6A8E] italic whitespace-pre-wrap">&quot;{{ $message->message }}&quot;</p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            @elseif($message->is_system)
                <div class="self-stretch flex items-center gap-3 my-2 px-2">
                    ...
                    <p class="text-[11px] text-[#5B6A8E] text-center max-w-[70%] leading-relaxed">{{ $message->message }}</p>
                    <div class="flex-1 h-px bg-[#E2E4EC]"></div>
                </div>
            @else
                @php
                ...
                @if($isSelf)
                    ...
                    <div class="group relative max-w-[75%] self-end {{ $endsRun ? 'mb-1.5' : '' }}" data-bubble>
                        <div class="bg-[#060D26] text-white rounded-2xl {{ $startsRun ? 'rounded-tr-sm' : '' }} px-4 py-2.5 shadow-sm cursor-default">
                            <p class="text-[13px] leading-relaxed whitespace-pre-wrap">{{ $message->message }}</p>
                        </div>
                        <p class="message-time absolute right-full top-1/2 -translate-y-1/2 mr-2 whitespace-nowrap text-[10px] tracking-wide text-[#5B6A8E] opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none"
                ...
                @else
                    <div class="flex items-end gap-2 self-start max-w-[85%] {{ $endsRun ? 'mb-1.5' : '' }}"
                        ...
                        <div class="group relative min-w-0" data-bubble>
                            <div class="bg-white text-[#060D26] border border-[#E2E4EC] rounded-2xl {{ $startsRun ? 'rounded-tl-sm' : '' }} px-4 py-2.5 shadow-sm cursor-default">
                                @if($startsRun)
                                    <p class="text-[11px] font-bold text-[#060D26] mb-1">
                                        {{ $message->sender->first_name }}
                                    </p>
                                @endif
                                <p class="text-[13px] leading-relaxed whitespace-pre-wrap">{{ $message->message }}</p>
                            </div>
                            <p class="message-time absolute left-full top-1/2 -translate-y-1/2 ml-2 whitespace-nowrap text-[10px] tracking-wide text-[#5B6A8E] opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none"
                                data-sent-at="{{ $message->sent_at->toIso8601String() }}"></p>
                        </div>
                    </div>
                @endif
    ...
    @if($conversation->isCancelled())
        <div class="px-5 py-2.5 bg-[#E2E4EC]/50 border-t border-[#E2E4EC] flex items-center gap-2.5 flex-shrink-0">
            <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#EF4444" stroke-width="2" class="shrink-0">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
            </svg>
            <p class="text-[12px] text-[#5B6A8E]">This conversation has been cancelled. You can no longer send messages here.</p>
        </div>
    @endif

    {{-- Message input --}}
    @if($conversation->isCancelled())
        <div class="px-5 py-3 bg-[#E2E4EC]/40 border-t border-[#E2E4EC] flex-shrink-0">
            <p class="text-center text-[12px] text-[#5B6A8E] font-medium">Messaging is disabled for cancelled conversations.</p>
        </div>
    @else
        <div class="px-5 py-3 bg-white border-t border-[#E2E4EC] flex-shrink-0">
