...
@section('content')
    @php
        ...
        $inputClass = 'h-11 w-full rounded-xl border border-[#5B6A8E]/30 px-3.5 text-[13.5px] text-[#060D26] placeholder-[#5B6A8E] focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/30 transition';
        ...
        <x-card class="mb-5">
            <div class="flex flex-col lg:flex-row lg:items-center gap-5">
                <div class="flex items-center gap-4 min-w-0 flex-1">
                    <div class="w-14 h-14 rounded-full bg-[#ECEEF6] flex items-center justify-center text-[18px] font-bold text-[#060D26] shrink-0">
                        {{ $initials ?: '?' }}
                    </div>
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <h1 class="text-xl font-normal text-[#060D26] truncate">
                        ...
                        <div class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5 text-[12.5px] text-[#5B6A8E]">
                ...
                <div class="flex flex-wrap items-center gap-2 shrink-0">
                    ...
                    @if($isActive)
                        <button type="button" @click="$dispatch('open-record-payment')"
                            class="h-11 px-5 inline-flex items-center gap-2 rounded-full bg-[#060D26] text-white text-sm font-semibold hover:brightness-95 transition-all duration-200 cursor-pointer">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                @if($monthlyTransactions->isNotEmpty())
                    ...
                    <x-card flush x-data="{ moreRows: false }">
                        <div class="px-5 sm:px-6 py-4 border-b border-[#E2E4EC]">
                            <h2 class="text-[15px] font-normal text-[#060D26]">Rent payments</h2>
                ...
                @if($otherCharges->isNotEmpty())
                    <x-card flush x-data="{ moreRows: false }">
                        <div class="px-5 sm:px-6 py-4 border-b border-[#E2E4EC]">
                            <h2 class="text-[15px] font-normal text-[#060D26]">Payment transactions</h2>
                ...
                @if($voidedTransactions->isNotEmpty())
                    <x-card flush x-data="{ moreRows: false }">
                        <div class="px-5 sm:px-6 py-4 border-b border-[#E2E4EC]">
                            <h2 class="text-[15px] font-normal text-[#060D26]">Voided entries</h2>
                                        ...
                                        <tr @if($loop->index >= 6) x-show="moreRows" x-cloak @endif>
                                            ...
                                            <td class="px-4 py-3.5">
                                                ...
                                                @if($voided->replacements->isNotEmpty())
                                                    <span class="inline-flex items-center h-5 px-2 mt-1 rounded-full border border-[#C9A84C]/25 bg-[#ECEEF6] text-[#060D26] text-[10px] font-bold">
                                                        Corrected
                                                    </span>
                                                @endif
                                            </td>
            ...
            <div class="space-y-5">
                ...
                <x-card>
                    ...
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div class="min-w-0">
                            <p class="text-[13.5px] font-bold text-[#060D26] truncate">{{ $unit->unit_label ?? 'No unit' }}</p>
                            <p class="text-[12px] text-[#5B6A8E] truncate">{{ $reservation->property->title ?? '' }}</p>
                        </div>
                    </div>
                    ...
                    <dl class="space-y-2.5 text-[13px]">
                        ...
                        @endif
                        <div class="flex items-center justify-between gap-3">
                            <dt class="text-[#5B6A8E]">Due day</dt>
                            <dd class="font-semibold text-[#060D26]">Day {{ $summary['dueDay'] }} of the month</dd>
                        </div>
                        <div class="flex items-center justify-between gap-3">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <dt class="text-[#5B6A8E]">Occupants</dt>
                            <dd class="font-semibold text-[#060D26]">{{ $reservation->occupants_count ?? '—' }}</dd>
                        </div>
                    </dl>
                    ...
                    @if($reservation->remarks)
                        <div class="mt-4 pt-4 border-t border-[#E2E4EC]">
                            ...
                            <p class="text-[12.5px] text-[#5B6A8E] leading-relaxed">{{ $reservation->remarks }}</p>
                        </div>
                    @endif
                </x-card>
