        ...
        <div class="flex gap-1.5 flex-1">
            <template x-for="i in 5" :key="i">
                <div class="h-1 flex-1 rounded-full transition-colors duration-300"
                    :class="step >= i ? 'bg-[#060D26]' : 'bg-[#E2E4EC]'"></div>
            </template>
        </div>
        <span class="text-xs font-semibold text-[#5B6A8E] whitespace-nowrap">Step <span x-text="step"></span> of 5</span>
    </div>
</div>

{{-- ── Desktop: vertical rail ─────────────────────────── --}}
<aside class="hidden lg:block lg:sticky lg:top-8">
    <p class="text-[11px] font-bold uppercase tracking-[0.11em] text-[#060D26]">Become a landlord</p>
    <p class="mt-1 text-xs text-[#5B6A8E] leading-relaxed">About 4 minutes. Your ID is stored privately and only an
        admin reviews it.</p>

    <ol class="mt-5 space-y-0.5">
        @foreach ($steps as $s)
            <li class="flex items-start gap-3 px-3 py-2.5 rounded-xl transition-colors duration-200"
                :class="step === {{ $s['n'] }} ? 'bg-white shadow-[0_1px_3px_rgba(6,13,38,0.06)]' : ''">
                <span
                    ...
                    :class="step > {{ $s['n'] }}
                        ...
                        : (step === {{ $s['n'] }}
                            ? 'bg-[#060D26] border-[#FF8A66] text-white'
                ...
                <span class="min-w-0">
                    <span class="block text-[13px] leading-snug transition-colors duration-200"
                        :class="step >= {{ $s['n'] }} ? 'text-[#060D26] font-semibold' : 'text-[#5B6A8E] font-medium'">
                        {{ $s['label'] }}
                    </span>
                    @if ($s['n'] === 1)
                        <span x-show="step > 1 && idType" class="block text-[11.5px] text-[#5B6A8E] truncate"
                            x-text="idType"></span>
                    @elseif ($s['n'] === 2)
                        <span x-show="step > 2" class="block text-[11.5px] text-[#5B6A8E]"
                            x-text="needsBack ? 'Front and back' : 'Data page'"></span>
                    @elseif ($s['n'] === 3)
                        <span x-show="step > 3" class="block text-[11.5px] text-[#5B6A8E]"
    ...
    <div class="mt-5 pt-4 border-t border-[#E2E4EC]">
        <p class="text-xs text-[#5B6A8E] leading-relaxed">Rather do this on your phone?
            <button type="button"
                ...
                @click="send()" class="text-left font-semibold text-[#060D26] hover:text-[#060D26] transition-colors"
