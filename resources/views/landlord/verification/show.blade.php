...
@section('content')
    <div class="max-w-3xl mx-auto px-4 py-12">
        ...
        <a href="{{ route('dashboard') }}"
            class="inline-flex items-center gap-1 text-sm text-[#64748B] hover:text-[#1F2937] mb-6">
        ...
        <h1 class="text-2xl font-semibold text-[#1F2937] mb-1">Landlord Verification</h1>
        <p class="text-sm text-[#64748B] mb-8">Status of your identity verification application.</p>
            ...
            <div class="rounded-xl border border-[#EEF8F8] bg-white p-5 sm:p-8">
                <div class="flex items-start gap-4">
                    <div class="flex-shrink-0 w-12 h-12 rounded-full bg-[#EEF8F8] flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="#2AA7A1" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <div>
                        <h2 class="text-lg font-semibold text-[#1F2937]">You're a verified landlord</h2>
                        @if ($verification->reviewed_at)
                            <p class="text-sm text-[#64748B] mt-1">
                                Approved on {{ \Carbon\Carbon::parse($verification->reviewed_at)->format('F j, Y') }}
                            </p>
                        @endif
                        <p class="text-sm text-[#1F2937] mt-3">
                ...
                <div class="mt-6 flex items-center gap-4">
                    <a href="{{ route('landlord.listings.index') }}"
                        class="inline-flex items-center px-4 py-2 rounded-md bg-[#2AA7A1] text-white text-sm font-medium brightness-95 hover:brightness-90">
                        Go to your listings
                    </a>
                    <a href="{{ route('verifications.document', $verification) }}"
                        class="text-sm text-[#64748B] hover:underline">
            ...
            <div class="rounded-xl border border-[#EF4444]/30 bg-white p-5 sm:p-8">
                <div class="flex items-start gap-4">
                    <div class="flex-shrink-0 w-12 h-12 rounded-full bg-[#EF4444]/10 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="#EF4444" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <div>
                        <h2 class="text-lg font-semibold text-[#1F2937]">Application rejected</h2>
                        @if ($verification->reviewed_at)
                            <p class="text-sm text-[#64748B] mt-1">
                ...
                @if ($verification->admin_notes)
                    <div class="mt-5 rounded-md bg-[#EEF8F8]/40 border border-[#EEF8F8] p-4">
                        <p class="text-xs font-medium uppercase tracking-wide text-[#64748B] mb-1">Reason from admin</p>
        ...
        @else {{-- Pending --}}
            ...
            <div class="rounded-xl border border-[#EEF8F8] bg-white p-5 sm:p-8">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-[#1F2937]">Application under review</h2>
