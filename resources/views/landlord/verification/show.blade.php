    ...
    <div class="min-h-[calc(100vh-72px)] py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl mx-auto" x-data="{ previewImage: null }">
            ...
            <a href="{{ auth()->user()->homeRoute() }}"
               class="inline-flex items-center gap-1.5 text-sm text-[#8a6e1e] hover:text-[#060D26] mb-6 transition-colors">
            ...
            {{-- Header --}}
            <h1 class="text-2xl font-normal text-[#060D26] mb-1">Landlord verification</h1>
            <p class="text-sm text-[#5B6A8E] mb-6">Status of your identity verification application.</p>
            ...
            @if ($verification->verification_status === 'Approved')
                <div class="rounded-lg border border-[#22C55E]/30 bg-[#22C55E]/[0.07] p-4 mb-6">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-[#060D26]">You're a verified landlord</p>
                            ...
                            <a href="{{ route('landlord.properties.index') }}"
                               class="inline-flex items-center gap-1.5 mt-3 px-4 py-2 rounded-lg text-sm font-semibold text-white bg-[#060D26] hover:brightness-95 transition-all">
                                Go to your listings
                                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            @elseif ($verification->verification_status === 'Rejected')
                <div class="rounded-lg border border-[#EF4444]/30 bg-[#EF4444]/[0.07] p-4 mb-6">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-[#060D26]">Application not approved</p>
                            <p class="text-sm text-[#5B6A8E] mt-1">
                            ...
                            @if ($verification->admin_notes)
                                <div class="mt-3 rounded-lg bg-white/60 border border-[#EF4444]/10 p-3">
                                    <p class="text-xs font-semibold text-[#5B6A8E] mb-1">Reason</p>
                                    <p class="text-sm text-[#060D26]">{{ $verification->admin_notes }}</p>
                                </div>
                            @endif
            ...
            @else {{-- Pending --}}
                <div class="rounded-lg border border-[#FBBF24]/30 bg-[#FBBF24]/[0.10] p-4 mb-6">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-[#060D26]">Application under review</p>
                            <p class="text-sm text-[#5B6A8E] mt-1">
                ...
                <div class="lg:col-span-2 space-y-4">
                    ...
                    <x-card flush class="p-5 sm:p-6">
                        <div class="flex items-center gap-2 mb-4">
                            <svg class="w-5 h-5 text-[#5B6A8E]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Zm6-10.125a1.875 1.875 0 1 1-3.75 0 1.875 1.875 0 0 1 3.75 0Zm1.294 6.336a6.721 6.721 0 0 1-3.17.789 6.721 6.721 0 0 1-3.168-.789 3.376 3.376 0 0 1 6.338 0Z" />
                            </svg>
                            <h2 class="text-base font-normal text-[#060D26]">Identity verification</h2>
                        </div>

                        {{-- ID info grid --}}
                        <div class="grid grid-cols-[110px_1fr] gap-x-3 gap-y-2 text-sm mb-5">
                            <span class="text-xs text-[#5B6A8E]">ID type</span>
                            <span class="text-sm text-[#060D26]">{{ $verification->id_type }}</span>

                            @if ($verification->ocr_name)
                                <span class="text-xs text-[#5B6A8E]">Name on ID</span>
                                <span class="text-sm text-[#060D26]">{{ $verification->ocr_name }}</span>
                            @endif

                            @if ($verification->id_number)
                                <span class="text-xs text-[#5B6A8E]">ID number</span>
                                <span class="text-sm text-[#060D26]">{{ $verification->id_number }}</span>
                            @endif
                        </div>
                            ...
                            <div class="flex flex-col gap-2 mb-5">
                                @if ($verification->ocr_status === 'pass')
                                    <div class="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#22C55E]/[0.07]">
                                        ...
                                        <span class="text-xs text-[#060D26]">Name matched your account</span>
                                    </div>
                                @elseif ($verification->ocr_status === 'partial')
                                    <div class="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#FBBF24]/[0.10]">
                                        ...
                                        <span class="text-xs text-[#060D26]">Partial name match — admin will verify</span>
                                    </div>
                                @else
                                    <div class="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#EF4444]/[0.07]">
                                        ...
                                        <span class="text-xs text-[#060D26]">Name not verified — admin will check manually</span>
                                    </div>
                                @endif
                            </div>
                        @endif
                        ...
                        <p class="text-xs font-semibold text-[#5B6A8E] mb-2">Submitted photos</p>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-[#5B6A8E] mt-1.5 text-center">ID front</p>
                            </div>
                            ...
                            @if ($verification->id_back)
                                <div>
                                    ...
                                    <p class="text-xs text-[#5B6A8E] mt-1.5 text-center">ID back</p>
                                </div>
                            @else
                                <div>
                                    <div class="rounded-lg border border-dashed border-[#E2E4EC] bg-[#F7F8FC] aspect-[4/3] flex items-center justify-center">
                                        <span class="text-xs text-[#5B6A8E]">N/A</span>
                                    </div>
                                    <p class="text-xs text-[#5B6A8E] mt-1.5 text-center">ID back</p>
                                </div>
                            @endif
                            ...
                            <div>
                                ...
                                <p class="text-xs text-[#5B6A8E] mt-1.5 text-center">Selfie</p>
                            </div>
                        </div>
                    </x-card>
                    ...
                    @if ($verification->business_name)
                        <x-card flush class="p-5 sm:p-6">
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <h2 class="text-base font-normal text-[#060D26]">Business details</h2>
                            </div>

                            <div class="grid grid-cols-[110px_1fr] gap-x-3 gap-y-2 text-sm">
                                <span class="text-xs text-[#5B6A8E]">Business name</span>
                                <span class="text-sm text-[#060D26]">{{ $verification->business_name }}</span>

                                @if ($verification->description)
                                    <span class="text-xs text-[#5B6A8E]">Description</span>
                                    <span class="text-sm text-[#060D26]">{{ $verification->description }}</span>
                                @endif

                                <span class="text-xs text-[#5B6A8E]">Contact</span>
                                <span class="text-sm text-[#060D26]">{{ $verification->contact_number }}</span>

                                <span class="text-xs text-[#5B6A8E]">Address</span>
                                <span class="text-sm text-[#060D26]">{{ $verification->business_address }}</span>
                            </div>
                        </x-card>
                    @endif
                </div>
                ...
                <div class="lg:col-span-1">
                    <x-card flush class="p-5 sm:p-6">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <h2 class="text-base font-normal text-[#060D26]">Timeline</h2>
                        </div>

                        <div class="flex flex-col pl-3 border-l-2 border-[#E2E4EC]">
                            {{-- Submitted --}}
                            <div class="relative pb-5 pl-5">
                                <div class="absolute -left-[7px] top-0.5 w-3 h-3 rounded-full bg-[#060D26]"></div>
                                <p class="text-sm font-semibold text-[#060D26]">Submitted</p>
                                <p class="text-xs text-[#5B6A8E] mt-0.5">{{ $verification->submitted_at->format('M j, Y \a\t g:i A') }}</p>
                            </div>

                            @if ($verification->verification_status === 'Approved')
                                <div class="relative pl-5">
                                    <div class="absolute -left-[7px] top-0.5 w-3 h-3 rounded-full bg-[#22C55E]"></div>
                                    <p class="text-sm font-semibold text-[#060D26]">Approved</p>
                                    <p class="text-xs text-[#5B6A8E] mt-0.5">{{ $verification->reviewed_at->format('M j, Y \a\t g:i A') }}</p>
                                </div>

                            @elseif ($verification->verification_status === 'Rejected')
                                <div class="relative pl-5">
                                    <div class="absolute -left-[7px] top-0.5 w-3 h-3 rounded-full bg-[#EF4444]"></div>
                                    <p class="text-sm font-semibold text-[#060D26]">Not approved</p>
                                    <p class="text-xs text-[#5B6A8E] mt-0.5">{{ $verification->reviewed_at->format('M j, Y \a\t g:i A') }}</p>
                                    @if ($verification->admin_notes)
                                        <p class="text-xs text-[#5B6A8E] mt-1">{{ $verification->admin_notes }}</p>
                                    @endif
                                </div>

                            @else
                                <div class="relative pl-5">
                                    <div class="absolute -left-[7px] top-0.5 w-3 h-3 rounded-full border-2 border-[#E2E4EC] bg-white"></div>
                                    <p class="text-sm font-semibold text-[#5B6A8E]">Admin review</p>
                                    <p class="text-xs text-[#5B6A8E] mt-0.5">Waiting for review</p>
                                </div>
                            @endif
                        </div>
                    </x-card>
