...
@section('content')
    <div class="max-w-3xl mx-auto px-4 py-12">
        ...
        <a href="{{ route('dashboard') }}"
            class="inline-flex items-center gap-1 text-sm text-[#9B9F98] hover:text-[#2A2523] mb-6">
        ...
        <h1 class="text-2xl font-semibold text-[#2A2523] mb-1">Landlord Verification</h1>
        <p class="text-sm text-[#9B9F98] mb-8">Status of your identity verification application.</p>

        @if ($verification->verification_status === 'Approved')

            <div class="rounded-xl border border-[#D7E8F3] bg-white p-8">
                <div class="flex items-start gap-4">
                    <div class="flex-shrink-0 w-12 h-12 rounded-full bg-[#D7E8F3] flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="#61B2F0" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <div>
                        <h2 class="text-lg font-semibold text-[#2A2523]">You're a verified landlord</h2>
                        @if ($verification->reviewed_at)
                            <p class="text-sm text-[#9B9F98] mt-1">
                                Approved on {{ \Carbon\Carbon::parse($verification->reviewed_at)->format('F j, Y') }}
                            </p>
                        @endif
                        <p class="text-sm text-[#2A2523] mt-3">
                ...
                <div class="mt-6 flex items-center gap-4">
                    <a href="{{ route('landlord.listings.index') }}"
                        class="inline-flex items-center px-4 py-2 rounded-md bg-[#61B2F0] text-white text-sm font-medium brightness-95 hover:brightness-90">
                        Go to your listings
                    </a>
        ...
        @elseif ($verification->verification_status === 'Rejected')
            ...
            <div class="rounded-xl border border-[#BD5434]/30 bg-white p-8">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-[#2A2523]">Application rejected</h2>
                        @if ($verification->reviewed_at)
                            <p class="text-sm text-[#9B9F98] mt-1">
                ...
                @if ($verification->admin_notes)
                    <div class="mt-5 rounded-md bg-[#D7E8F3]/40 border border-[#D7E8F3] p-4">
        ...
        @else {{-- Pending --}}
            ...
            <div class="rounded-xl border border-[#D7E8F3] bg-white p-8">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-[#2A2523]">Application under review</h2>
