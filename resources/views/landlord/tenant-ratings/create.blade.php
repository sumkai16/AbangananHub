...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <a href="{{ route('landlord.reservations.index', ['status' => 'Occupied']) }}"
            class="inline-flex items-center gap-1.5 text-sm font-medium text-[#5B6A8E] hover:text-[#060D26] mb-6">
        ...
        <h1 class="text-2xl font-normal text-[#060D26] mb-1">Rate Tenant</h1>
        <p class="text-sm text-[#5B6A8E] mb-8">Share your experience with this tenant to help other landlords.</p>

        {{-- Tenant & reservation context card --}}
        <div class="bg-white border border-[#E2E4EC] rounded-2xl shadow-[0_1px_3px_rgba(6,13,38,0.06)] p-5 mb-6">
            <div class="flex items-center gap-4">
                <div
                    class="w-12 h-12 rounded-full bg-[#060D26] text-white text-lg font-bold flex items-center justify-center shrink-0">
                    {{ strtoupper(substr($reservation->tenant->first_name ?? '?', 0, 1)) }}
                </div>
                <div class="min-w-0">
                    <p class="text-base font-semibold text-[#060D26]">
                        {{ $reservation->tenant->first_name }} {{ $reservation->tenant->last_name }}
                    </p>
                    <p class="text-[13px] text-[#5B6A8E]">
        ...
        <form action="{{ route('landlord.reservations.rateTenant.store', $reservation) }}" method="POST"
            class="bg-white border border-[#E2E4EC] rounded-2xl shadow-[0_1px_3px_rgba(6,13,38,0.06)] p-6" x-data="{ rating: 0, hovered: 0 }">
            @csrf

            {{-- Star picker --}}
            <div class="mb-6">
                <label class="block text-sm font-semibold text-[#060D26] mb-3">Rating</label>
                    ...
                    @for($i = 1; $i <= 5; $i++)
                        <button type="button" @click="rating = {{ $i }}" @mouseenter="hovered = {{ $i }}"
                            ...
                            <svg width="32" height="32" viewBox="0 0 24 24"
                                ...
                                :stroke="(hovered || rating) >= {{ $i }} ? '#FBBF24' : '#E2E4EC'" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M11.48 3.499a.562.562 0 0 1 1.04 0l2.125 5.111a.563.563 0 0 0 .475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 0 0-.182.557l1.285 5.385a.562.562 0 0 1-.84.61l-4.725-2.885a.563.563 0 0 0-.586 0L6.982 20.54a.562.562 0 0 1-.84-.61l1.285-5.386a.562.562 0 0 0-.182-.557l-4.204-3.602a.562.562 0 0 1 .321-.988l5.518-.442a.563.563 0 0 0 .475-.345L11.48 3.5z" />
                            </svg>
                        </button>
                    @endfor
                    <span class="ml-2 text-sm text-[#5B6A8E]" x-show="rating > 0" x-cloak>
            ...
            <div class="flex items-center gap-3">
                <button type="submit" x-bind:disabled="rating === 0"
                    class="px-6 py-2.5 bg-[#060D26] text-white text-sm font-semibold rounded-xl hover:brightness-95 transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed">
                    Submit Rating
                </button>
                <a href="{{ route('landlord.reservations.index', ['status' => 'Occupied']) }}"
                    class="px-5 py-2.5 text-sm font-medium text-[#5B6A8E] hover:text-[#060D26] transition-colors duration-150">
                    Cancel
                </a>
            </div>
        </form>
