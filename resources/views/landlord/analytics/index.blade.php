...
@section('content')
    @php
        ...
        $statusStyles = [
            ...
            'Maintenance' => ['tile' => 'border-[#E2E4EC] bg-[#F7F8FC]',           'dot' => 'bg-[#94A3B8]', 'verb' => 'is now under maintenance'],
        ];
    @endphp
    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-10">
        ...
        <x-page-header title="Analytics" subtitle="Overview of your rental business performance.">
            ...
            <x-slot:actions>
                <form method="GET" action="{{ route('landlord.analytics.index') }}">
                    ...
                    <div class="relative">
                        <x-styled-select name="range" id="range" :options="$ranges" :selected="$rangeKey" :autosubmit="true"
                            class="h-10 pl-9 pr-4 rounded-xl border border-[#E2E4EC] bg-white text-[13px] font-medium text-[#060D26]" />
                        <svg class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#5B6A8E] z-10" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <a href="{{ route('landlord.analytics.export', ['range' => $rangeKey]) }}"
                    class="h-10 px-4 rounded-xl bg-[#060D26] text-[#F7F4ED] text-[13px] font-bold hover:brightness-95 cursor-pointer transition-all duration-200 inline-flex items-center gap-1.5">
        ...
        @php
            $cards = [
                ['label' => 'Total Properties', 'value' => number_format($stats['properties']), 'sub' => 'Active properties', 'tint' => '#060D26', 'box' => 'bg-[#060D26]/10', 'delta' => null,
                ...
                ['label' => 'Total Units', 'value' => number_format($stats['units']), 'sub' => 'All rental units', 'tint' => '#5B6A8E', 'box' => 'bg-[#5B6A8E]/10', 'delta' => null,
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-3 mb-3">
            ...
            {{-- Occupancy Overview --}}
            <x-card>
                <h2 class="text-[14px] font-normal text-[#060D26] mb-3">Occupancy Overview</h2>
                @if($occupancyTotal === 0)
                    <p class="text-[12.5px] text-[#5B6A8E] py-8 text-center">No units yet.</p>
                @else
                    <div class="relative h-[150px] mb-3">
                        <canvas id="occupancyChart"></canvas>
                        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span class="text-xl font-bold text-[#060D26]">{{ $stats['occupancyRate'] }}%</span>
                            <span class="text-[10px] text-[#5B6A8E]">Occupancy</span>
                        </div>
                    </div>
                    <ul class="flex flex-col gap-1.5">
                        @foreach($occupancyBreakdown as $slice)
                            <li class="flex items-center gap-2 text-[11.5px]">
                                ...
                                <span class="text-[#060D26] flex-1">{{ $slice['label'] }}</span>
                                <span class="text-[#5B6A8E]">{{ $slice['count'] }}</span>
                                <span class="text-[#060D26] font-semibold w-11 text-right">
            ...
            <x-card class="lg:col-span-1 xl:col-span-2">
                <div class="flex items-center justify-between mb-3">
                    <h2 class="text-[14px] font-normal text-[#060D26]">Revenue Overview</h2>
                    <span class="text-[11px] text-[#5B6A8E]">Last 6 months</span>
                </div>
                @if($revenueTrend->sum('value') <= 0)
                    <div class="h-[210px] flex flex-col items-center justify-center text-center">
                        <p class="text-[13px] font-semibold text-[#060D26]">No revenue recorded yet</p>
                        <p class="text-[12px] text-[#5B6A8E] mt-1 max-w-[260px]">Revenue appears once a tenant completes payment on a signed agreement.</p>
                    </div>
                @else
            ...
            {{-- Revenue by Property --}}
            <x-card>
                <h2 class="text-[14px] font-normal text-[#060D26] mb-3">Revenue by Property</h2>
                @if($revenueTotal <= 0)
                    <p class="text-[12.5px] text-[#5B6A8E] py-8 text-center">No revenue in this period.</p>
                @else
                    <div class="relative h-[150px] mb-3">
                        <canvas id="revenueByPropertyChart"></canvas>
                        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span class="text-[15px] font-bold text-[#060D26]">&#8369;{{ number_format($revenueTotal, 0) }}</span>
                            <span class="text-[10px] text-[#5B6A8E]">Total</span>
                        </div>
                    </div>
                    <ul class="flex flex-col gap-1.5">
                        @foreach($topSlices as $i => $slice)
                            <li class="flex items-center gap-2 text-[11.5px]">
                                ...
                                <span class="text-[#060D26] flex-1 truncate">{{ $slice['title'] }}</span>
                                <span class="text-[#060D26] font-semibold shrink-0">&#8369;{{ number_format($slice['revenue'], 0) }}</span>
                            </li>
                        @endforeach
                        @if($othersTotal > 0)
                            <li class="flex items-center gap-2 text-[11.5px]">
                                <span class="w-2 h-2 rounded-full shrink-0 {{ $sliceDotClasses[4] }}"></span>
                                <span class="text-[#060D26] flex-1">Others</span>
                                <span class="text-[#060D26] font-semibold shrink-0">&#8369;{{ number_format($othersTotal, 0) }}</span>
                            </li>
                        @endif
                    </ul>
                @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
            ...
            <x-card>
                <h2 class="text-[14px] font-normal text-[#060D26] mb-3">Occupancy by Property</h2>
                @if($perProperty->isEmpty())
                    <p class="text-[12.5px] text-[#5B6A8E] py-6 text-center">No properties yet.</p>
                @else
                    <div class="flex flex-col gap-3">
                        @foreach($perProperty->take(5) as $row)
                            <div>
                                <div class="flex items-baseline justify-between gap-2 mb-1">
                                    <p class="text-[12px] text-[#060D26] truncate">{{ $row['title'] }}</p>
                                    <p class="text-[11.5px] font-bold text-[#060D26] shrink-0">{{ $row['rate'] }}%</p>
                                </div>
                                <div class="flex h-2 rounded-full overflow-hidden bg-[#E2E4EC]">
                                    @if($row['total'] > 0)
            ...
            <x-card>
                <div class="flex items-center justify-between mb-3">
                    <h2 class="text-[14px] font-normal text-[#060D26]">Reservations Overview</h2>
                    <a href="{{ route('landlord.reservations.index') }}"
                        class="text-[11.5px] font-bold text-[#060D26] hover:underline">View All</a>
                </div>
                @if($reservationTotal === 0)
                    <p class="text-[12.5px] text-[#5B6A8E] py-8 text-center">No reservations in this period.</p>
                @else
                    <div class="relative h-[150px] mb-3">
                        <canvas id="reservationChart"></canvas>
                        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span class="text-xl font-bold text-[#060D26]">{{ $reservationTotal }}</span>
                            <span class="text-[10px] text-[#5B6A8E]">Total</span>
                        </div>
                    </div>
                    <ul class="flex flex-col gap-1.5">
                        @foreach($reservationBreakdown as $slice)
                            <li class="flex items-center gap-2 text-[11.5px]">
                                <span class="w-2 h-2 rounded-full shrink-0 {{ $dotClassFor[$slice['color']] ?? 'bg-[#94A3B8]' }}"></span>
                                <span class="text-[#060D26] flex-1">{{ $slice['label'] }}</span>
                                <span class="text-[#060D26] font-semibold">
            ...
            <x-card flush class="lg:col-span-2 xl:col-span-1">
                <div class="px-5 sm:px-6 pt-5 sm:pt-6 pb-3">
                    <h2 class="text-[14px] font-normal text-[#060D26]">Top Performing Properties</h2>
                </div>
                @if($perProperty->isEmpty())
                    <p class="text-[12.5px] text-[#5B6A8E] py-8 text-center">No properties yet.</p>
                @else
                    <div class="overflow-x-auto">
                        <table class="w-full min-w-[320px]">
                            <thead>
                                <tr class="border-y border-[#E2E4EC] bg-[#F7F8FC]">
                                    <th class="text-left text-[10px] font-bold text-[#5B6A8E] uppercase tracking-wider px-5 sm:px-6 py-2">Property</th>
                                    <th class="text-right text-[10px] font-bold text-[#5B6A8E] uppercase tracking-wider px-3 py-2">Occupancy</th>
                                    <th class="text-right text-[10px] font-bold text-[#5B6A8E] uppercase tracking-wider px-5 sm:px-6 py-2">Revenue</th>
                                </tr>
                            </thead>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 mt-3">
            ...
            <x-card flush class="p-5">
                <div class="flex items-center justify-between gap-2 mb-4">
                    <h2 class="text-[14px] font-normal text-[#060D26]">Vacancy Watch</h2>
                    <span class="shrink-0 rounded-full border border-[#E2E4EC] bg-[#F7F8FC] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#5B6A8E]">As of today</span>
                </div>
                ...
                @if($vacancy['count'] === 0)
                    <div class="flex flex-col items-center justify-center py-8 text-center">
                        ...
                        <p class="text-[13px] font-semibold text-[#060D26]">No vacant units</p>
                        <p class="text-[11.5px] text-[#5B6A8E] mt-1 max-w-[230px]">Every unit is reserved, occupied, or under maintenance.</p>
                    </div>
                @else
                    {{-- The headline: rent the portfolio is not earning. This is
                         the one figure that makes an empty unit feel like a cost
                         rather than a status. --}}
                    <div class="rounded-xl bg-[#F7F8FC] border border-[#E2E4EC] px-4 py-3.5 mb-4">
                        <p class="text-[11px] font-bold text-[#5B6A8E] uppercase tracking-wide">Rent not being earned</p>
                        <p class="text-[22px] font-extrabold text-[#B45309] mt-1 leading-none">
                            ₱{{ number_format($vacancy['idle_rent']) }}<span class="text-[12px] font-semibold text-[#5B6A8E]"> / month</span>
                        </p>
                    ...
                    <p class="text-[11px] font-bold text-[#5B6A8E] uppercase tracking-wide mb-2.5">Empty longest</p>
                    <div class="space-y-2">
                        @foreach($vacancy['units'] as $unit)
                            ...
                            <a href="{{ $unit['edit_url'] }}"
                                class="flex items-center gap-3 rounded-xl border border-[#E2E4EC] px-3 py-2.5 hover:bg-[#F7F8FC] transition-colors duration-150">
                                <div class="min-w-0 flex-1">
                                    <p class="text-[12.5px] font-semibold text-[#060D26] truncate">{{ $unit['label'] }}</p>
                                    <p class="text-[11px] text-[#5B6A8E] truncate">{{ $unit['property'] }}</p>
                                </div>
                                <div class="shrink-0 text-right">
                                    <span class="inline-block rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $tone[0] }} {{ $tone[1] }}">
                                        {{ $days }}{{ $days === 1 ? ' day' : ' days' }}
                                    </span>
                                    <p class="text-[11px] text-[#5B6A8E] mt-1">₱{{ number_format($unit['rent']) }}/mo</p>
                                </div>
                            </a>
                        @endforeach
                    </div>

                    <p class="text-[11px] text-[#5B6A8E] mt-3">
            ...
            <x-card flush class="p-5">
                <div class="flex items-center justify-between gap-2 mb-4">
                    <h2 class="text-[14px] font-normal text-[#060D26]">Recent Activity</h2>
                    <span class="shrink-0 rounded-full border border-[#E2E4EC] bg-[#F7F8FC] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#5B6A8E]">As of today</span>
                </div>
                @if($recentActivities->isEmpty())
                    <p class="text-[12px] text-[#5B6A8E] py-4 text-center">No occupancy changes recorded yet.</p>
                @else
                    <div class="relative">
                        {{-- Timeline rule, stopped short of the last dot's centre. --}}
                        <span class="absolute left-[13px] top-3 bottom-6 w-px bg-[#E2E4EC]" aria-hidden="true"></span>
                            ...
                            @foreach($recentActivities as $activity)
                                ...
                                <div class="relative flex items-start gap-3">
                                    ...
                                    <div class="flex-1 min-w-0 pt-0.5">
                                        <p class="text-[12.5px] text-[#060D26] leading-snug">
        ...
        <p class="text-[11px] text-[#5B6A8E] text-center mt-5">
                ...
                new Chart(revEl, {
                    ...
                    options: {
                        ...
                        scales: {
                            y: {
                                beginAtZero: true,
                                ticks: { callback: peso, font: { size: 10 }, color: '#5B6A8E' },
                                grid: { color: '#E2E4EC' },
                            },
                            x: {
                                ticks: { font: { size: 10 }, color: '#5B6A8E' },
                                grid: { display: false },
                            },
                        },
                    },
