...
@section('content')
    <div class="max-w-[1400px] mx-auto px-4 sm:px-8 lg:px-[50px] py-8">

        {{-- Success flash --}}
        @if(session('success'))
            <div class="bg-green-50 border border-green-200 text-green-800 rounded-xl px-4 py-3 text-[13px] font-medium flex items-center justify-between shadow-sm mb-6">
        ...
        <div class="bg-white rounded-xl border border-[#E2E8F0] p-6 sm:p-8 mb-6">
            <div class="flex flex-col sm:flex-row gap-6 items-start">
                ...
                <div class="flex-shrink-0">
                    ...
                    @else
                        <div class="w-24 h-24 rounded-full bg-[#2AA7A1] flex items-center justify-center text-white text-[32px] font-bold">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <h1 class="text-[22px] font-bold text-[#1F2937] leading-tight">{{ $user->first_name }} {{ $user->last_name }}</h1>
                        ...
                        @if($isOwner)
                            <a href="{{ route('landlord.profile.edit') }}" class="flex-shrink-0 flex items-center gap-2 px-4 py-2 border border-[#E2E8F0] rounded-lg text-[13px] font-semibold text-[#1F2937] hover:brightness-95 bg-white transition-all">
                        ...
                        @else
                            <div class="flex items-center gap-2 flex-shrink-0">
                                @auth
                                    ...
                                    <a href="{{ route('reports.create', ['user_id' => $user->user_id]) }}" class="flex items-center gap-2 px-4 py-2 rounded-full border border-[#E2E8F0] text-[13px] font-semibold text-[#64748B] hover:text-[#1F2937] hover:border-[#64748B] transition-all">
                                        <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                    ...
                    <div class="flex items-center gap-2 mt-4 flex-wrap">
                        <span class="bg-[#EEF8F8] text-[#156F8C] text-[12px] font-medium px-3 py-1 rounded-full">Landlord</span>
                        @if($verification && $verification->verification_status === 'Approved')
                            <span class="bg-green-50 text-green-700 text-[12px] font-medium px-3 py-1 rounded-full flex items-center gap-1">
            ...
            <div class="bg-white rounded-xl border border-[#E2E8F0] p-5 mb-6">
                <div class="flex items-center gap-3 mb-3">
                    @if($business->logo_url)
                        <img src="{{ $business->logo_url }}" alt="{{ $business->business_name }}" class="w-10 h-10 rounded-lg object-cover">
                    @else
                        <div class="w-10 h-10 rounded-lg bg-[#EEF8F8] flex items-center justify-center flex-shrink-0">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            <div class="bg-white rounded-xl border border-[#E2E8F0] p-4 text-center">
                <p class="text-[13px] text-[#64748B] mb-1">Properties</p>
                <p class="text-[22px] font-bold text-[#1F2937]">{{ $properties->count() }}</p>
            </div>
            <div class="bg-white rounded-xl border border-[#E2E8F0] p-4 text-center">
                <p class="text-[13px] text-[#64748B] mb-1">Total units</p>
                <p class="text-[22px] font-bold text-[#1F2937]">{{ $totalUnits }}</p>
            </div>
            <div class="bg-white rounded-xl border border-[#E2E8F0] p-4 text-center">
                <p class="text-[13px] text-[#64748B] mb-1">Occupied</p>
                <p class="text-[22px] font-bold text-[#1F2937]">{{ $occupiedUnits }}</p>
            </div>
            <div class="bg-white rounded-xl border border-[#E2E8F0] p-4 text-center">
                <p class="text-[13px] text-[#64748B] mb-1">Avg rating</p>
                <p class="text-[22px] font-bold text-[#1F2937] flex items-center justify-center gap-1">
                    @if($averageRating)
                        ...
                        {{ $averageRating }}
                    @else
                        <span class="text-[#64748B] text-[14px] font-normal">N/A</span>
                    @endif
                </p>
            </div>
        </div>
            ...
            @if($properties->count())
                    ...
                    @foreach($properties->take(6) as $property)
                        <a href="{{ route('properties.show', $property) }}" class="group">
                            <div class="rounded-xl overflow-hidden">
                                ...
                                @if($thumb)
                                    <img src="{{ $thumb->media_url }}" alt="{{ $property->title }}" class="w-full h-[180px] object-cover rounded-xl group-hover:brightness-95 transition-all">
                                @else
                                    <div class="w-full h-[180px] bg-[#EEF8F8] rounded-xl flex items-center justify-center">
                                        <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="#2AA7A1" stroke-width="1.5">
                ...
                @if($properties->count() > 6)
                    <div class="text-center mt-5">
                       <a href="{{ $isOwner ? route('landlord.properties.index') : route('properties.index', ['landlord' => $user->user_id]) }}" class="inline-flex items-center gap-2 px-6 py-2.5 border border-[#E2E8F0] rounded-full text-[13px] font-semibold text-[#1F2937] bg-white hover:brightness-95 transition-all">
                            Show all {{ $properties->count() }} properties
                        </a>
                    </div>
                @endif
            @else
                <div class="bg-white rounded-xl border border-[#E2E8F0] py-8 text-center">
                    <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="#E2E8F0" stroke-width="1.5" class="mx-auto mb-2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 21v-4.875c0-.621.504-1.125 1.125-1.125h5.25c.621 0 1.125.504 1.125 1.125V21m0 0h4.5V3.545M15.75 21H8.25m6.386-8.818a3.375 3.375 0 11-6.747-.248l-.006.248a3.375 3.375 0 116.747.248z" />
                    </svg>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if($review->landlord_reply)
                                <div class="mt-2 pl-3 border-l-2 border-[#E2E8F0]">
                                  <p class="text-[11px] font-semibold text-[#64748B]">{{ $isOwner ? 'Your reply' : 'Landlord reply' }}</p>
