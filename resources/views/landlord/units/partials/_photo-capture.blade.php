...
<x-card flush class="p-6" x-data="{ tab: 'live' }">
    <div class="flex items-center gap-2.5 mb-3">
        ...
        <div>
            <h2 class="text-[13px] font-normal text-[#060D26]">
    ...
    <div class="mb-4 px-3.5 py-3 rounded-xl bg-[#ECEEF6] border border-[#DA8E77]/20 flex items-start gap-2.5">
        <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="#DA8E77" stroke-width="2" class="shrink-0 mt-0.5">
    ...
    {{-- Tabs --}}
    <div class="inline-flex rounded-xl border border-[#5B6A8E]/25 bg-[#F7F8FC] p-1 mb-4">
        <button type="button" x-on:click="tab = 'live'"
            :class="tab === 'live' ? 'bg-white shadow-sm text-[#060D26]' : 'text-[#5B6A8E]'"
        ...
        <button type="button" x-on:click="tab = 'upload'"
            :class="tab === 'upload' ? 'bg-white shadow-sm text-[#060D26]' : 'text-[#5B6A8E]'"
            class="inline-flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-[12.5px] font-semibold transition-colors duration-150 cursor-pointer">
            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    {{-- Live Capture panel --}}
    <div x-show="tab === 'live'">
        <div id="camera-shell" class="rounded-xl border border-[#5B6A8E]/25 bg-[#060D26] overflow-hidden relative aspect-video flex items-center justify-center">
            ...
            <div id="camera-idle" class="text-center px-6">
                ...
                <button type="button" id="camera-enable"
                    class="h-9 px-4 inline-flex items-center gap-1.5 rounded-full bg-[#DA8E77] text-[#060D26] text-[12.5px] font-semibold hover:bg-[#C97A61] transition-all duration-200">
                    Enable camera
                </button>
            </div>
            ...
            <button type="button" id="camera-capture"
                ...
                <span class="w-10 h-10 rounded-full border-2 border-[#060D26]"></span>
            </button>
        </div>
    ...
    <div x-show="tab === 'upload'" x-cloak>
        <div id="photo-dropzone"
            class="rounded-xl border-2 border-dashed border-[#5B6A8E]/30 bg-[#F7F8FC] px-6 py-8 text-center cursor-pointer hover:border-[#DA8E77]/60 transition-colors duration-200">
            ...
            <p class="text-[13px] font-semibold text-[#060D26]">Click to select photos</p>
            <p class="text-[11.5px] text-[#5B6A8E] mt-0.5">JPEG, PNG or WEBP</p>
    ...
    <div class="mt-4 flex items-center gap-2 text-[12px]">
        <span id="live-count-badge" class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 font-semibold bg-[#DA8E77]/[0.08] text-[#060D26] border border-[#DA8E77]/25">
    ...
    (function () {
        ...
        function render() {
            ...
            photos.forEach((p) => {
                ...
                card.className = 'rounded-xl overflow-hidden border border-[#E2E4EC] bg-white';

                const media = document.createElement('div');
                media.className = 'relative aspect-video bg-[#F7F8FC]';
                ...
                sourceTag.className = 'absolute top-1.5 left-1.5 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold '
                    + (p.source === 'camera' ? 'bg-[#DA8E77] text-[#060D26]' : 'bg-white/90 text-[#060D26] border border-[#E2E4EC]');
                ...
                cap.className = 'h-9 w-full rounded-lg border border-[#5B6A8E]/25 px-2.5 text-[12px] text-[#060D26] placeholder-[#5B6A8E] focus:outline-none focus:ring-2 focus:ring-[#DA8E77]/30 transition';
                cap.value = p.caption;
                ...
                rm.type = 'button';
                rm.className = 'absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-white/90 border border-[#E2E4EC] flex items-center justify-center text-[#EF4444] hover:brightness-95 transition';
