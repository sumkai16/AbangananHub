...
@section('content')
    ...
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-[50px] py-8">
        ...
        <div class="mb-6">
            <h1 class="text-[22px] font-bold text-[#1F2937]">Good morning, {{ $greetingName }}!</h1>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-4">
                ...
                <p class="text-[24px] font-bold text-[#1F2937]">{{ $totalProperties }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-4">
                <p class="text-[12px] text-[#64748B] font-medium mb-1">Total Units</p>
                <p class="text-[24px] font-bold text-[#1F2937]">{{ $totalUnits }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-4">
                <p class="text-[12px] text-[#64748B] font-medium mb-1">Occupied Units</p>
                <p class="text-[24px] font-bold text-[#1F2937]">{{ $occupiedUnits }}</p>
            ...
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-4">
                ...
                <p class="text-[24px] font-bold text-[#1F2937]">{{ $availableUnits }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-4 col-span-2 md:col-span-1">
                ...
                <p class="text-[24px] font-bold text-[#1F2937]">{{ $totalTenants }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-5">
                <h2 class="text-[14px] font-bold text-[#1F2937] mb-4">Occupancy Overview</h2>

                <div class="relative flex items-center justify-center mb-4">
                    <svg width="160" height="160" viewBox="0 0 160 160" class="-rotate-90">
                        <circle cx="80" cy="80" r="{{ $radius }}" fill="none" stroke="#EEF8F8" stroke-width="16" />
                        ...
                        @endif
                        @if($reservedUnits > 0)
                            <circle cx="80" cy="80" r="{{ $radius }}" fill="none" stroke="#2AA7A1" stroke-width="16"
                ...
                <div class="space-y-2 text-[12px]">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-semibold text-[#1F2937]">{{ $occupiedUnits }} ({{ $occupiedPct }}%)</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="flex items-center gap-2"><span
                                class="w-2.5 h-2.5 rounded-full bg-[#64748B]"></span>Available</span>
                        <span class="font-semibold text-[#1F2937]">{{ $availableUnits }} ({{ $availablePct }}%)</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="flex items-center gap-2"><span
            ...
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-5 lg:col-span-2 flex flex-col">
                <h2 class="text-[14px] font-bold text-[#1F2937] mb-4">Occupancy Trend</h2>
                <div class="flex-1 flex flex-col items-center justify-center text-center py-10">
                    ...
                    <p class="text-[13px] font-semibold text-[#1F2937]">Coming soon</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-2xl border border-[#64748B]/15 p-5">
                <h2 class="text-[14px] font-bold text-[#1F2937] mb-4">Monthly Revenue</h2>
