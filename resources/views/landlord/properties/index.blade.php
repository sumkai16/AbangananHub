    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-16"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <a href="{{ route('properties.create') }}"
                class="inline-flex items-center justify-center gap-2 h-11 px-6 rounded-full bg-[#DA8E77] hover:bg-[#C97A61] text-[#060D26] text-sm font-semibold shadow-sm transition-all duration-200 shrink-0">
        ...
        @if($properties->isEmpty())
            <div class="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p class="text-sm text-[#5B6A8E] mt-1 max-w-xs">Add your first property to start receiving reservations from
                    tenants.</p>
                <a href="{{ route('properties.create') }}"
                    class="mt-5 inline-flex items-center gap-2 h-10 px-5 rounded-full bg-[#DA8E77] text-[#060D26] text-sm font-semibold hover:bg-[#C97A61] transition-all duration-200">
        ...
        @else
            @php
                // Derived per-property data shared by both the card grid and the table view
                $derived = [];
                            ...
                            <div class="flex items-center gap-2 pt-1 mt-auto">
                                @if($property->isDraft())
                                    <a href="{{ route('properties.wizard.resume', $property) }}"
                                        class="flex-1 h-9 flex items-center justify-center gap-1.5 rounded-full bg-[#DA8E77] text-[#060D26] text-[12px] font-semibold hover:bg-[#C97A61] transition-all duration-200">
                                        <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
                                        </svg>
                                        Continue Setup
                                    ...
                                    <form method="POST" action="{{ route('properties.destroy', $property) }}"
                                          data-confirm="Delete Draft Property?"
                                          data-confirm-type="error"
                                          data-confirm-message="Are you sure you want to delete this draft? All information, photos, units, and setup data associated with this draft will be permanently removed."
