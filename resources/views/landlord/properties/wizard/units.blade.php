...
<div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-[1200px] mx-auto">
        ...
        <div class="grid lg:grid-cols-[248px_minmax(0,1fr)] lg:gap-11">
            ...
            <x-property-wizard-stepper current="units" :property="$property" :checklist="$checklist ?? null" />

            <div class="min-w-0">
                <div class="max-w-2xl flex items-start justify-between gap-4 flex-wrap">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-[0.11em] text-[#060D26]">Step 5 of 6</p>
                        <h1 class="mt-1.5 text-2xl font-normal tracking-tight text-[#060D26]">Add your units</h1>
                        <p class="mt-2 text-sm text-[#5B6A8E] leading-relaxed max-w-md">
                    ...
                    <a href="{{ route('landlord.properties.units.create', $property) }}?from=wizard"
                        class="inline-flex items-center gap-1.5 h-10 px-5 rounded-xl bg-[#DA8E77] text-[#060D26] text-sm font-semibold hover:bg-[#C97A61] transition-all duration-200 shrink-0">
                ...
                <div class="mt-7 max-w-2xl">
                    @if($property->units->isEmpty())
                        <div class="border border-dashed border-[#E2E4EC] rounded-2xl p-10 text-center">
                            ...
                            <a href="{{ route('landlord.properties.units.create', $property) }}?from=wizard"
                                class="inline-flex items-center gap-1.5 h-10 px-5 rounded-xl bg-[#DA8E77] text-[#060D26] text-sm font-semibold hover:bg-[#C97A61] transition-all duration-200">
                                Add Unit
                            </a>
                        </div>
                    @else
                        <div class="space-y-2.5">
                            @foreach($property->units as $unit)
                                ...
                                @endphp
                                <div class="border border-[#E2E4EC] rounded-xl p-3.5 flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            <p class="text-[13.5px] font-semibold text-[#060D26] truncate">{{ $unit->unit_label }}</p>
                                            <p class="text-[12px] text-[#5B6A8E] flex items-center gap-1.5">
                                    ...
                                    <a href="{{ route('landlord.properties.units.edit', [$property, $unit]) }}?from=wizard"
                                        class="text-[12.5px] font-semibold text-[#060D26] hover:text-[#04091B] transition-colors shrink-0">
                                        Edit
                                    </a>
                                </div>
                            @endforeach
                ...
                <div class="mt-7 pt-5 border-t border-[#E2E4EC] max-w-2xl">
                    @unless($checklist['units']['complete'])
                        <p class="mb-3 text-[12.5px] text-[#5B6A8E]">Add at least one unit above to continue.</p>
                    @endunless
                    <div class="flex items-center gap-3">
                        ...
                        @else
                            <button type="button" disabled title="Add at least one unit to continue"
                                class="ml-auto px-9 py-3 rounded-xl text-sm font-semibold text-[#060D26] bg-[#DA8E77] opacity-40 cursor-not-allowed">
                                Continue to Review
                            </button>
                        @endif
                    </div>
