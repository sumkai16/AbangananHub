...
<div class="border border-[#E2E4EC] rounded-xl p-4 {{ $document && !$document->isRequested() ? 'bg-white' : ($required ? 'bg-[#F7F8FC]' : 'bg-white') }}">
    ...
    @if($document && ! $document->isRequested())
        <div class="flex items-center justify-between gap-3 mt-3 pt-3 border-t border-[#E2E4EC]">
            <a href="{{ route('landlord.properties.documents.preview', [$property, $document]) }}" target="_blank"
               class="inline-flex items-center gap-1.5 text-[12.5px] font-medium text-[#060D26] hover:text-[#04091B] transition-colors truncate">
            ...
            <div class="flex items-center gap-3 shrink-0">
                <button type="button" class="text-[11.5px] font-semibold text-[#060D26] hover:text-[#04091B] transition-colors" @click="openType = openType === '{{ $type }}' ? null : '{{ $type }}'">View</button>
...
=======
            <label class="flex items-center gap-3 px-3.5 py-3 rounded-xl border-2 border-dashed border-[#E2E4EC] hover:border-[#FF8A66] bg-[#F7F8FC] cursor-pointer transition-colors duration-150 group">
>>>>>>> 092fb1454a20ae889717d4d8b1bee67f9c0c8eaa
                ...
                <span class="text-[12.5px] text-[#060D26] min-w-0">
                    <span class="font-semibold text-[#060D26]">Click to upload</span>
                    <span class="text-[#5B6A8E]"> — PDF, JPG, or PNG</span>
                </span>
