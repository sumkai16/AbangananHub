...
<div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-[1200px] mx-auto">
        ...
        <div class="grid lg:grid-cols-[248px_minmax(0,1fr)] lg:gap-11">
            ...
            <div class="min-w-0">
                <p class="text-[11px] font-bold uppercase tracking-[0.11em] text-[#060D26]">Step 1 of 6</p>
                <h1 class="mt-1.5 text-2xl font-normal tracking-tight text-[#060D26]">Tell us about the property</h1>
                ...
                <form method="POST" action="{{ $property ? route('properties.wizard.info.update', $property) : route('properties.wizard.info.store') }}"
                    ...
                    <div>
                        <label class="block text-[13px] font-semibold text-[#060D26] mb-1.5">Property type</label>
                        <x-styled-select name="property_type" :options="['Bedspace' => 'Bedspace', 'Room' => 'Room', 'Apartment' => 'Apartment', 'House' => 'House']"
                            ...
                            class="w-full h-12 px-4 rounded-xl border {{ $errors->has('property_type') ? 'border-[#EF4444]/40' : 'border-[#E2E4EC]' }} text-[14px] text-[#060D26]" />
                        @error('property_type')<p class="text-xs text-[#EF4444] mt-1.5">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="block text-[13px] font-semibold text-[#060D26] mb-1.5">Living arrangement <span class="font-normal text-[#94A3B8]">(optional)</span></label>
                        <x-styled-select name="living_arrangement" :options="['Private' => 'Private', 'Shared' => 'Shared', 'Mixed' => 'Mixed', 'Female only' => 'Female only', 'Male only' => 'Male only', 'Couples allowed' => 'Couples allowed', 'Family-friendly' => 'Family-friendly']"
                            ...
                            class="w-full h-12 px-4 rounded-xl border {{ $errors->has('living_arrangement') ? 'border-[#EF4444]/40' : 'border-[#E2E4EC]' }} text-[14px] text-[#060D26]" />
                    ...
                    <div>
                        <label class="block text-[13px] font-semibold text-[#060D26] mb-1.5">Utilities / included charges</label>
