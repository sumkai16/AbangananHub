                ...
                <form method="POST" action="{{ $property ? route('properties.wizard.location.update', $property) : route('properties.wizard.location.store') }}"
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="block text-[13px] font-semibold text-[#060D26]">Pin your location</label>
                            <button type="button" id="location-picker-expand" class="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#060D26] hover:text-[#04091B] transition-colors">
                        ...
                        <div id="location-picker-map-wrapper" class="relative rounded-xl overflow-hidden border @error('latitude') border-[#EF4444]/40 @elseif ($errors->has('longitude')) border-[#EF4444]/40 @else border-[#E2E4EC] @enderror">
                        ...
                        <div id="location-picker-placeholder" class="hidden h-[300px] w-full rounded-xl border border-dashed border-[#E2E4EC] bg-[#F7F8FC] items-center justify-center text-[12px] font-medium text-[#94A3B8] text-center px-4">
                            Map opened in full screen — tap Done to bring it back here.
                        </div>
                        <div class="flex items-center justify-between gap-3 mt-2">
                            <p id="location-picker-address-line" class="text-[12px] text-[#5B6A8E] truncate">Address will appear here after pinning.</p>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-[13px] font-semibold text-[#060D26] mb-1.5">City / Municipality</label>
                            <div id="city-municipality-picker" data-lgus="{{ json_encode(config('cebu.lgus')) }}">
                                <x-styled-select name="city_municipality"
                                    ...
                                    class="w-full h-12 px-4 rounded-xl border {{ $errors->has('city_municipality') ? 'border-[#EF4444]/40' : 'border-[#E2E4EC]' }} text-[14px] text-[#060D26]" />
                            </div>
                    ...
                    <div class="pt-2 border-t border-[#E2E4EC]">
                        <label class="block text-[13px] font-semibold text-[#060D26] mt-6 mb-3">Photos</label>
                        ...
                        <div class="border-2 border-dashed @error('photos') border-[#EF4444]/30 @else border-[#E2E4EC] @enderror hover:border-[#C9A84C] rounded-2xl p-6 bg-[#F7F8FC] text-center transition-colors group">
                            <label class="cursor-pointer block">
                                ...
                                <span id="upload-label" data-default-label="Select photos to upload" class="text-[13.5px] font-semibold text-[#060D26] transition-colors">Select photos to upload</span>
                    ...
                    <div class="mt-7 pt-5 border-t border-[#E2E4EC] flex items-center gap-3">
                        ...
                        <button type="submit" :disabled="submitting"
                            class="ml-auto inline-flex items-center gap-2 px-9 py-3 rounded-xl text-sm font-semibold text-[#F7F4ED] bg-[#060D26] hover:brightness-95 transition-all duration-150 disabled:opacity-60 disabled:cursor-wait disabled:hover:brightness-100">
...
<div id="location-picker-modal" class="fixed inset-0 z-[200] hidden items-center justify-center p-4 sm:p-8" role="dialog" aria-modal="true" aria-label="Pin your property location" aria-hidden="true">
    ...
    <div id="location-picker-modal-panel" class="relative w-full max-w-5xl h-[85vh] bg-white rounded-2xl shadow-xl flex flex-col overflow-hidden opacity-0 translate-y-4 scale-95 transition-all duration-300">
        ...
        <div class="px-5 py-4 border-t border-[#E2E4EC] shrink-0 flex items-center justify-end">
            <button type="button" id="location-picker-modal-done" class="h-11 px-6 rounded-full bg-[#060D26] text-[#F7F4ED] font-bold text-[13.5px] shadow-sm hover:brightness-95 transition-all">
                Done
            </button>
        </div>
    </div>
