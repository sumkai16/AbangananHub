...
<div class="w-full px-4 sm:px-8 lg:px-[50px] py-8 pb-16">
    ...
    <a href="{{ route('landlord.properties.index') }}"
       class="inline-flex items-center gap-2 text-sm text-[#9B9F98] hover:text-[#0F172A] transition-colors duration-200 mb-6">
    ...
    <div class="flex flex-col lg:flex-row gap-6 mb-6">
            ...
            <div class="relative w-full aspect-[4/3] rounded-2xl overflow-hidden bg-[#F1F5F9]">
                ...
                @else
                    <div class="w-full h-full flex flex-col items-center justify-center gap-2">
                        <svg width="36" height="36" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2" class="text-[#9B9F98]/50">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5z"/>
                        </svg>
                        <span class="text-sm text-[#9B9F98]">No photos uploaded</span>
                    </div>
                @endif
            </div>
        ...
        <div class="flex-1 min-w-0 flex flex-col" x-data="{ moreOpen: false }">
            ...
            <div class="flex items-center justify-end gap-2 mb-4">
                <a href="{{ route('properties.edit', $property) }}"
                   class="inline-flex items-center gap-1.5 h-9 px-4 rounded-full border border-[#9B9F98]/30 text-[#0F172A] text-sm font-medium hover:bg-[#F1F5F9] transition-colors duration-200">
                ...
                <div class="relative">
                    <button @click="moreOpen = !moreOpen"
                            class="inline-flex items-center gap-1 h-9 px-3 rounded-full border border-[#9B9F98]/30 text-[#0F172A] text-sm font-medium hover:bg-[#F1F5F9] transition-colors duration-200">
                    ...
                    <div x-show="moreOpen" x-cloak @click.outside="moreOpen = false"
                        ...
                        <a href="{{ route('landlord.properties.units.index', $property) }}"
                           class="flex items-center gap-2 px-4 py-2.5 text-sm text-[#0F172A] hover:bg-[#F1F5F9] transition-colors">
                            <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" class="text-[#9B9F98]">
                        ...
                        <div class="h-px bg-[#9B9F98]/10 mx-3 my-1"></div>
            ...
            <p class="text-sm text-[#9B9F98] flex items-center gap-1.5 mb-3 flex-wrap">
            ...
            @if($property->amenities->isNotEmpty())
                <div class="flex flex-wrap gap-2 mt-auto pt-2">
                    ...
                    @if($property->amenities->count() > 5)
                        <span class="inline-flex items-center text-xs font-medium text-[#9B9F98] bg-[#F1F5F9] rounded-full px-3 py-1.5">
        ...
        <div class="w-full lg:w-56 shrink-0">
            <div class="bg-white rounded-2xl ring-1 ring-[#9B9F98]/15 p-4 space-y-3">
                <div class="flex items-center justify-between gap-2">
                    <h3 class="text-xs font-bold text-[#9B9F98] uppercase tracking-widest">Property Status</h3>
                    @php
                ...
                <div class="space-y-2.5 text-[13px]">
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-[#9B9F98]">Property ID</span>
                        <span class="font-semibold text-[#0F172A] font-mono text-xs">PRP-{{ str_pad($property->property_id, 4, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-[#9B9F98]">Total Units</span>
                        <span class="font-semibold text-[#0F172A]">{{ $unitStats['total'] }} units</span>
                    </div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-[#9B9F98]">Occupied</span>
                        <span class="font-semibold text-[#0F172A]">
                            {{ $unitStats['occupied'] }} units
                            @if($unitStats['total'] > 0)
                                <span class="text-[#9B9F98] font-normal">({{ round($unitStats['occupied'] / $unitStats['total'] * 100) }}%)</span>
                            @endif
                        </span>
                    </div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-[#9B9F98]">Available</span>
                        <span class="font-semibold text-emerald-600">
                            {{ $unitStats['available'] }} units
                            @if($unitStats['total'] > 0)
                                <span class="text-[#9B9F98] font-normal">({{ round($unitStats['available'] / $unitStats['total'] * 100) }}%)</span>
                            @endif
                        </span>
                    </div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-[#9B9F98]">Reserved</span>
                        <span class="font-semibold text-amber-500">
                            {{ $unitStats['reserved'] }} units
                            @if($unitStats['total'] > 0)
                                <span class="text-[#9B9F98] font-normal">({{ round($unitStats['reserved'] / $unitStats['total'] * 100) }}%)</span>
                            @endif
                        </span>
                    </div>
                    <div class="flex items-center justify-between gap-2">
                        <span class="text-[#9B9F98]">Created Date</span>
    ...
    <div x-data="{ tab: 'overview' }">
        ...
        {{-- Tab nav --}}
        <div class="flex items-center gap-1 border-b border-[#9B9F98]/20 mb-6 overflow-x-auto">
            @foreach([
                ...
                <button @click="tab = '{{ $t['key'] }}'"
                        :class="tab === '{{ $t['key'] }}' ? 'border-b-2 border-[#0F172A] text-[#0F172A] font-semibold' : 'text-[#9B9F98] hover:text-[#0F172A]'"
        ...
        <div x-show="tab === 'overview'" x-cloak>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                ...
                <div class="bg-white rounded-2xl ring-1 ring-[#9B9F98]/15 p-5">
                    <h3 class="text-sm font-bold text-[#0F172A] mb-4 flex items-center gap-2">
                        <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" class="text-[#9B9F98]">
                    ...
                    <div class="space-y-3 text-sm">
                        <div>
                            <p class="text-[10px] text-[#9B9F98] font-semibold uppercase tracking-wider mb-0.5">Property Type</p>
                            <p class="text-[#0F172A] font-semibold">{{ $property->property_type }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] text-[#9B9F98] font-semibold uppercase tracking-wider mb-0.5">Address</p>
                            <p class="text-[#0F172A] font-semibold">{{ $property->address }}</p>
                        </div>
                        @if($property->description)
                            <div>
                                <p class="text-[10px] text-[#9B9F98] font-semibold uppercase tracking-wider mb-0.5">Property Description</p>
                                <p class="text-[#0F172A]/75 leading-relaxed text-xs">{{ $property->description }}</p>
                            </div>
                        @endif
                        @if($property->amenities->isNotEmpty())
                            <div>
                                <p class="text-[10px] text-[#9B9F98] font-semibold uppercase tracking-wider mb-1.5">Amenities</p>
                                <div class="flex flex-wrap gap-1.5">
                                    @foreach($property->amenities->take(5) as $amenity)
                                        <span class="text-[11px] text-[#0F172A] bg-[#F1F5F9] rounded-full px-2.5 py-0.5">{{ $amenity->amenity_name }}</span>
                                    @endforeach
                                    @if($property->amenities->count() > 5)
                                        <span class="text-[11px] text-[#9B9F98] bg-[#F1F5F9] rounded-full px-2.5 py-0.5">+{{ $property->amenities->count() - 5 }}</span>
                                    @endif
                                </div>
                            </div>
                        @endif
                ...
                {{-- Location --}}
                <div class="bg-white rounded-2xl ring-1 ring-[#9B9F98]/15 p-5 flex flex-col">
                    ...
                    <div id="property-map"
                         ...
                         class="w-full flex-1 min-h-[160px] rounded-xl overflow-hidden border border-[#9B9F98]/20 bg-[#F1F5F9] mb-3">
                    </div>
                    <a href="{{ $mapsUrl }}" target="_blank" rel="noopener"
                       class="flex items-center justify-center gap-2 w-full h-9 rounded-xl border border-[#9B9F98]/30 text-sm text-[#0F172A] font-medium hover:bg-[#F1F5F9] transition-colors duration-200">
                        <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" class="text-[#9B9F98]">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0z"/>
                        </svg>
                    ...
                    <div class="flex items-center justify-center mb-4">
                        <div class="relative w-32 h-32">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                ...
                                <span class="text-[10px] text-[#9B9F98] mt-0.5">Total Units</span>
                            </div>
                        </div>
                    </div>
                        ...
                        @foreach($legend as $row)
                            <div class="flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[#9B9F98]">{{ $row['label'] }}</span>
                                </div>
                                <span class="font-semibold text-[#0F172A]">
            ...
            <div class="bg-white rounded-2xl ring-1 ring-[#9B9F98]/15 p-5">
                        ...
                        @foreach($recentUnits as $recentUnit)
                            <div class="flex items-start gap-3 p-3 rounded-xl bg-[#F1F5F9]/50">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] text-[#9B9F98] mt-0.5">{{ $recentUnit->updated_at->format('M d, Y \a\t h:i A') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="flex flex-col items-center justify-center py-8 text-center">
                        <svg width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" class="text-[#9B9F98]/40 mb-2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/>
                        </svg>
                        <p class="text-sm text-[#9B9F98]">No recent activity</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div x-show="tab === 'units'" x-cloak>
            @if($property->units->isEmpty())
                <div class="rounded-2xl border border-dashed border-[#9B9F98]/30 bg-[#F1F5F9]/40 flex flex-col items-center justify-center py-14 text-center">
                    <svg width="28" height="28" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2" class="text-[#9B9F98]/50 mb-3">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6zm0 9.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25zm9.75-9.75A2.25 2.25 0 0 1 15.75 3.75H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6zm0 9.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25z"/>
                    </svg>
                    <p class="text-sm font-semibold text-[#0F172A]">No units added yet</p>
                    <p class="text-xs text-[#9B9F98] mt-1 mb-4">Add units so tenants can reserve specific spaces.</p>
                    <a href="{{ route('landlord.properties.units.create', $property) }}"
            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($property->units as $unit)
                        ...
                        @endphp
                        <div class="flex flex-col rounded-2xl overflow-hidden bg-white ring-1 ring-[#9B9F98]/15 hover:shadow-md transition-shadow duration-300">
                            <div class="relative h-36 bg-[#F1F5F9] overflow-hidden">
                                ...
                                @else
                                    <div class="w-full h-full flex items-center justify-center">
                                        <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2" class="text-[#9B9F98]/40">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5z"/>
                                        </svg>
                                    </div>
                                @endif
                            ...
                            <div class="p-3 flex flex-col gap-2">
                                ...
                                <p class="text-[11px] text-[#9B9F98] flex items-center gap-1">
                                ...
                                <div class="flex items-center gap-2 pt-2 border-t border-[#9B9F98]/10">
                                    <a href="{{ route('landlord.properties.units.edit', [$property, $unit]) }}"
                                       class="flex-1 h-8 flex items-center justify-center gap-1 rounded-full border border-[#9B9F98]/30 text-[#0F172A] text-[11px] font-medium hover:bg-[#F1F5F9] transition-colors duration-200">
                                        <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931z"/>
                                        </svg>
                                        Edit
        ...
        <div x-show="tab === 'amenities'" x-cloak>
            @if($property->amenities->isEmpty())
                <div class="rounded-2xl border border-dashed border-[#9B9F98]/30 bg-[#F1F5F9]/40 flex flex-col items-center justify-center py-14 text-center">
                    ...
                    <p class="text-xs text-[#9B9F98] mt-1">Edit this property to add amenities.</p>
                </div>
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    @foreach($property->amenities as $amenity)
                        <div class="flex items-center gap-3 bg-white rounded-xl ring-1 ring-[#9B9F98]/15 px-4 py-3">
                            <div class="w-8 h-8 rounded-lg bg-[#DBEAFE] flex items-center justify-center shrink-0">
        ...
        <div x-show="tab === 'photos'" x-cloak>
            @if($images->isEmpty())
                <div class="rounded-2xl border border-dashed border-[#9B9F98]/30 bg-[#F1F5F9]/40 flex flex-col items-center justify-center py-14 text-center">
                    ...
                    <p class="text-xs text-[#9B9F98] mt-1">Edit this property to add photos.</p>
                </div>
            @else
        ...
        <div x-show="tab === 'reviews'" x-cloak>
            @if($reviews->isEmpty())
                <div class="rounded-2xl border border-dashed border-[#9B9F98]/30 bg-[#F1F5F9]/40 flex flex-col items-center justify-center py-14 text-center">
                    ...
                    <p class="text-xs text-[#9B9F98] mt-1">Reviews will appear here once tenants submit them.</p>
                </div>
            @else
                <div class="space-y-3">
                        ...
                        <div class="bg-white rounded-2xl ring-1 ring-[#9B9F98]/15 p-5">
                            <div class="flex items-start justify-between gap-4 mb-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-full bg-[#DBEAFE] flex items-center justify-center shrink-0 text-sm font-bold text-[#3B82F6]">
                                        {{ strtoupper(substr($review->tenant->first_name ?? '?', 0, 1)) }}
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-[#0F172A]">{{ $review->tenant->name ?? 'Tenant' }}</p>
                                        <p class="text-[11px] text-[#9B9F98]">{{ $review->created_at->format('M d, Y') }}</p>
                                    </div>
                                </div>
        ...
        <div x-show="tab === 'activity_log'" x-cloak>
            <div class="rounded-2xl border border-dashed border-[#9B9F98]/30 bg-[#F1F5F9]/40 flex flex-col items-center justify-center py-14 text-center">
                ...
                <p class="text-xs text-[#9B9F98] mt-1">A full history of actions on this property will appear here.</p>
            </div>
        </div>
