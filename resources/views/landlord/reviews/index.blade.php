    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6 items-start">
                ...
                <form method="GET" action="{{ route('landlord.reviews.index') }}">
                    <div class="flex flex-wrap items-center gap-2.5">
                        ...
                        <x-styled-select name="property" :options="$reviewsPropertyOptions" :selected="(string) request('property', '')"
                            class="h-11 pl-3 pr-9 rounded-xl border border-[#5B6A8E]/25 bg-[#F7F8FC] text-[13.5px] text-[#060D26] max-w-[200px]" />
                ...
                @if($reviews->isEmpty())
                    <x-card flush class="flex flex-col items-center justify-center py-16 px-6 text-center">
                        ...
                        <a href="{{ route('landlord.properties.index') }}"
                            class="mt-5 inline-flex items-center gap-1.5 h-10 px-5 rounded-xl bg-[#060D26] text-white text-[13px] font-semibold hover:brightness-95 transition-all duration-200">
                            ...
                            <x-card flush class="p-5">
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-full bg-[#ECEEF6] flex items-center justify-center text-[13px] font-bold text-[#060D26] shrink-0">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[12px] text-[#5B6A8E] flex items-center gap-1">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="flex items-center gap-0.5">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg class="w-4 h-4 {{ $i <= $review->rating ? 'text-[#B45309]' : 'text-[#E2E4EC]' }}"
                                        ...
                                        <span class="text-[12px] text-[#5B6A8E]">{{ $review->created_at->format('M d, Y') }}</span>
                                    </div>
                                </div>

                                {{-- Comment --}}
                                <p class="text-[13.5px] text-[#060D26] mt-3 leading-relaxed">{{ $review->review_comment }}</p>
                                ...
                                @if(!$review->landlord_reply)
                                    <div x-show="replyOpenId === {{ $review->review_id }}" x-cloak x-transition
                                        class="mt-3 pt-3 border-t border-[#5B6A8E]/10">
                                        <form method="POST" action="{{ route('landlord.reviews.reply', $review) }}" class="space-y-2.5">
                                            ...
                                            @endif
                                            <button type="submit"
                                                class="h-9 px-4 rounded-full bg-[#060D26] text-white text-[12.5px] font-semibold hover:brightness-95 transition-all duration-200">
                                                Submit Reply
                                            </button>
                                        </form>
                                    </div>
            ...
            <aside class="flex flex-col gap-4 lg:sticky lg:top-24">
                <x-card flush class="p-5">
                    <h3 class="text-[13px] font-normal text-[#060D26] mb-3.5">Build your reputation</h3>
                    <ul class="flex flex-col gap-3">
                        <li class="flex items-start gap-2.5">
                            ...
                            <span class="text-[12.5px] text-[#5B6A8E] leading-relaxed">Reply to every review — a thoughtful response shows future tenants you're engaged</span>
                        </li>
                        <li class="flex items-start gap-2.5">
                            <span class="w-5 h-5 rounded-full bg-[#FBBF24]/[0.10] flex items-center justify-center shrink-0 mt-0.5">
                                <svg width="10" height="10" viewBox="0 0 24 24" fill="#F59E0B"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
                            </span>
                            <span class="text-[12.5px] text-[#5B6A8E] leading-relaxed">Keep responses professional, even for critical feedback</span>
                        </li>
                        <li class="flex items-start gap-2.5">
                            <span class="w-5 h-5 rounded-full bg-[#FBBF24]/[0.10] flex items-center justify-center shrink-0 mt-0.5">
                                <svg width="10" height="10" viewBox="0 0 24 24" fill="#F59E0B"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
                            </span>
                            <span class="text-[12.5px] text-[#5B6A8E] leading-relaxed">A strong rating increases visibility in property search</span>
                        </li>
                    </ul>
                </x-card>
