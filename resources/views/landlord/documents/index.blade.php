...
<div class="max-w-[880px] mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-16">
    ...
    <div class="flex flex-wrap items-center gap-1.5 text-sm text-[#5B6A8E] mb-2">
        ...
        <a href="{{ route('landlord.properties.show', $property) }}" class="hover:text-[#060D26] transition-colors duration-200">{{ $property->title }}</a>
        <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
        </svg>
        <span class="text-[#060D26] font-medium">Documents</span>
    </div>

    {{-- Header --}}
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-normal text-[#060D26] leading-tight">Verification Documents</h1>
            <p class="text-sm text-[#5B6A8E] mt-1 max-w-lg">Proof of ownership, tax declaration, and permits for this property. These are only ever seen by admins — never shown to renters.</p>
        </div>
        ...
        @if($availableTypes->isNotEmpty() && $documents->isNotEmpty())
            <button type="button" @click="uploadOpen = true"
                    class="inline-flex items-center gap-1.5 h-10 px-5 rounded-full bg-[#060D26] text-white text-sm font-semibold hover:brightness-95 transition-all duration-200 shrink-0">
    ...
    @if($documents->count() > 1)
        ...
        <div class="flex flex-wrap items-center gap-x-5 gap-y-1.5 mb-6">
            @foreach($strip as $s)
                <span class="inline-flex items-center gap-1.5 text-xs text-[#5B6A8E]">
                    <span class="w-1.5 h-1.5 rounded-full shrink-0" style="background-color: {{ $s['dot'] }}"></span>
                    <span class="font-semibold text-[#060D26]">{{ $s['value'] }}</span> {{ $s['label'] }}
                </span>
            @endforeach
        </div>
    @endif
    ...
    @if($documents->isEmpty())
        ...
        <div class="bg-white border border-[#E2E4EC] rounded-2xl p-14 text-center shadow-[0_1px_3px_rgba(6,13,38,0.06)]">
            ...
            <button type="button" @click="uploadOpen = true"
                    class="inline-flex items-center gap-1.5 h-10 px-5 rounded-full bg-[#060D26] text-white text-sm font-semibold hover:brightness-95 transition-all duration-200">
                <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
                </svg>
                Upload a document
    ...
    @else
        ...
        <div class="space-y-8">
            ...
            @if($requested->isNotEmpty())
                <div>
                    <h2 class="text-xs font-normal text-[#5B6A8E] uppercase tracking-widest mb-3">Requested by admin — awaiting upload</h2>
                    <div class="space-y-3">
                        @foreach($requested as $document)
                            <div x-data="{ open: false }" class="bg-[#3B82F6]/[0.05] border border-[#3B82F6]/20 rounded-2xl p-5">
                                <div class="flex items-center justify-between gap-3">
                                    <p class="text-sm font-semibold text-[#060D26]">{{ $document->document_type }}</p>
                                    <div class="flex items-center gap-2 shrink-0">
                                        <x-document-status-badge :document="$document" />
                                        <button type="button" @click="open = !open"
                                                class="inline-flex items-center gap-1.5 h-8 px-3 rounded-full bg-[#060D26] text-white text-xs font-semibold hover:brightness-95 transition-all duration-200">
                                            Upload file
                                        </button>
                                    </div>
                                </div>
                                ...
                                <form x-show="open" x-cloak method="POST" action="{{ route('landlord.properties.documents.replace', [$property, $document]) }}"
                                    ...
                                    <div class="flex-1 min-w-[180px]">
                                        <label class="block text-xs font-medium text-[#5B6A8E] mb-1">File (PDF or image)</label>
                                        <input type="file" name="file" required accept=".pdf,.jpg,.jpeg,.png,.webp"
                                               class="w-full text-sm border border-[#E2E4EC] rounded-lg px-3 py-2 bg-white file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-[#ECEEF6] file:text-[#060D26] file:text-xs file:font-medium">
                                    </div>
                                    <div class="min-w-[120px]">
                                        <label class="block text-xs font-medium text-[#5B6A8E] mb-1">Doc. no.</label>
                                        <input type="text" name="document_number" maxlength="100" class="w-full text-sm border border-[#E2E4EC] rounded-lg px-3 py-2 bg-white">
                                    </div>
                                    ...
                                    <button type="submit" class="h-9 px-4 rounded-full bg-[#060D26] text-white text-sm font-semibold hover:brightness-95 transition-all duration-200">
                                        Upload
                                    </button>
                                </form>
                            </div>
            ...
            @if($submitted->isNotEmpty())
                <div>
                    <h2 class="text-xs font-normal text-[#5B6A8E] uppercase tracking-widest mb-3">Submitted</h2>
                    <div class="space-y-4">
                            ...
                            <div class="bg-white border border-[#E2E4EC] rounded-2xl shadow-[0_1px_3px_rgba(6,13,38,0.06)] p-5">
                                ...
                                <div class="flex flex-wrap items-center gap-2" x-data="{ resubmitting: false }">
                                    <a href="{{ route('landlord.properties.documents.download', [$property, $document]) }}"
                                       class="inline-flex items-center gap-1.5 h-8 px-3 rounded-full border border-[#5B6A8E]/30 text-[#060D26] text-xs font-medium hover:bg-[#ECEEF6] transition-colors duration-200">
                                        Download
                                    </a>
                                    ...
                                    @if($document->status === 'Rejected')
                                        <form x-show="resubmitting" x-cloak method="POST" action="{{ route('landlord.properties.documents.replace', [$property, $document]) }}"
                                              enctype="multipart/form-data" class="w-full mt-3 pt-3 border-t border-[#E2E4EC] flex flex-wrap items-end gap-3">
                                            @csrf
                                            <input type="hidden" name="document_type" value="{{ $document->document_type }}">
                                            <div class="flex-1 min-w-[180px]">
                                                <label class="block text-xs font-medium text-[#5B6A8E] mb-1">New file (PDF or image)</label>
                                                <input type="file" name="file" required accept=".pdf,.jpg,.jpeg,.png,.webp"
                                                       class="w-full text-sm border border-[#E2E4EC] rounded-lg px-3 py-2 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-[#ECEEF6] file:text-[#060D26] file:text-xs file:font-medium">
                                            </div>
                                            <div class="min-w-[120px]">
                                                <label class="block text-xs font-medium text-[#5B6A8E] mb-1">Document no.</label>
                                                <input type="text" name="document_number" value="{{ $document->document_number }}" maxlength="100" class="w-full text-sm border border-[#E2E4EC] rounded-lg px-3 py-2">
                                            </div>
                                            <div class="min-w-[140px]">
...
<div x-show="uploadOpen" x-cloak
    ...
    <div x-show="uploadOpen"
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-[#E2E4EC]">
            <h2 class="text-base font-normal text-[#060D26]">Upload a document</h2>
            <button type="button" @click="uploadOpen = false"
                    class="w-8 h-8 rounded-full flex items-center justify-center text-[#5B6A8E] hover:bg-[#ECEEF6] hover:text-[#060D26] transition-colors">
        ...
        <form method="POST" action="{{ route('landlord.properties.documents.store', $property) }}" enctype="multipart/form-data"
            ...
            @csrf
            <div>
                <label class="block text-xs font-medium text-[#5B6A8E] mb-1">Document type</label>
                @php
                ...
                <x-styled-select name="document_type" required :options="$documentTypeOptions" :selected="old('document_type', '')"
                    class="h-10 w-full rounded-lg border border-[#E2E4EC] px-3 text-sm text-[#060D26] bg-white" />
            </div>
            <div>
                <label class="block text-xs font-medium text-[#5B6A8E] mb-1">File (PDF or image, max 10MB)</label>
                <input type="file" name="file" required accept=".pdf,.jpg,.jpeg,.png,.webp"
                       class="w-full text-sm border border-[#E2E4EC] rounded-lg px-3 py-2.5 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-[#ECEEF6] file:text-[#060D26] file:text-xs file:font-medium">
            </div>
            <div>
                <label class="block text-xs font-medium text-[#5B6A8E] mb-1">Document no. (optional)</label>
                <input type="text" name="document_number" maxlength="100" class="w-full text-sm border border-[#E2E4EC] rounded-lg px-3 py-2.5">
            </div>
            <div>
                <label class="block text-xs font-medium text-[#5B6A8E] mb-1">Expiry date (optional)</label>
                <input type="date" name="expiry_date" class="w-full text-sm border border-[#E2E4EC] rounded-lg px-3 py-2.5">
            </div>

            <div class="pt-2 flex gap-2">
                <button type="submit" class="flex-1 h-10 rounded-full bg-[#060D26] text-white text-sm font-semibold hover:brightness-95 transition-all duration-200">
                    Upload document
                </button>
                <button type="button" @click="uploadOpen = false"
                        class="h-10 px-4 rounded-full border border-[#E2E4EC] text-sm font-medium text-[#5B6A8E] hover:text-[#060D26] transition-colors">
                    Cancel
                </button>
            </div>
        </form>
