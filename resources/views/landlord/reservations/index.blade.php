    ...
    <div class="bg-white border border-[#9B9F98]/15 rounded-2xl overflow-hidden">
        @if($reservations->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
                <svg width="40" height="40" fill="none" viewBox="0 0 24 24" stroke="#9B9F98" stroke-width="1.5" class="mb-3">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                </svg>
                <p class="text-sm font-semibold text-[#2A2523] mb-1">No reservations {{ $status !== 'all' ? 'with this status' : 'yet' }}</p>
                        ...
                        <tr class="border-t border-[#9B9F98]/10 hover:bg-[#F7F8FA] transition-colors duration-150">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-sm font-medium text-[#2A2523] truncate">{{ $reservation->property->title }}</p>
                                        <p class="text-[12px] text-[#9B9F98] truncate">{{ $reservation->unit->unit_label ?? 'No unit' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-4">
                                <p class="text-sm text-[#2A2523]">{{ $reservation->reservation_date?->format('M d, Y') ?? '—' }}</p>
                            </td>
                            <td class="px-5 py-4">
                                <p class="text-sm text-[#2A2523]">{{ $reservation->duration_of_stay ?? '—' }}</p>
                            </td>
                            <td class="px-5 py-4">
                                <span class="inline-flex px-2.5 py-1 rounded-full text-[11px] font-bold {{ $statusStyles[$reservation->reservation_status] ?? '' }}">
                                    ...
                                    @elseif($reservation->isApproved())
                                        <div class="relative" x-data="{ menuOpen: false }">
                                            <button @click="menuOpen = !menuOpen" @click.outside="menuOpen = false"
                                                class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#F0EDE8] text-[#9B9F98] transition-colors duration-150">
                                            ...
                                            <div x-show="menuOpen" x-cloak
                                                class="absolute right-0 top-[calc(100%+6px)] w-44 bg-white rounded-xl shadow-lg border border-[#9B9F98]/15 py-1.5 z-20">
                                                <form action="{{ route('landlord.reservations.cancel', $reservation) }}" method="POST"
                                                    ...
                                                    @csrf @method('PATCH')
                                                    <button type="submit"
                                                        class="w-full text-left px-4 py-2 text-[13px] font-medium text-[#BD5434] hover:bg-red-50">
                                                        Cancel reservation
                                                    </button>
                                                </form>
                                            </div>
