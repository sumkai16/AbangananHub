...
@section('content')
    <div class="max-w-5xl mx-auto px-4 sm:px-6 py-10 pb-16" x-data="{ rejectModalOpen: false, activeReservationId: null }">
        ...
        <div class="mb-8 flex items-center justify-between">
            ...
            <span
                class="text-[13px] font-semibold text-gray-400 bg-gray-50 border border-gray-200/60 px-3 py-1.5 rounded-xl">
        ...
        @if (session('success'))
            <div
                class="mb-6 px-4 py-3 rounded-xl bg-emerald-50 text-emerald-800 text-[14px] font-medium border border-emerald-100 flex items-center gap-2">
                {{ session('success') }}
            </div>
        @endif
        ...
        @else
            <div class="space-y-12">
                @foreach($reservations as $reservation)
                    ...
                    <div
                        class="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all duration-300">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6 h-auto md:h-56">
                            ...
                            <div
                                class="md:col-span-2 relative rounded-2xl overflow-hidden bg-gray-100 border border-gray-150 h-48 md:h-full">
                                @if($thumbnail)
                                    <img src="{{ $thumbnail->media_url }}" alt="{{ $reservation->property->title }}"
                                        class="w-full h-full object-cover">
                                @else
                                    <div class="w-full h-full flex items-center justify-center bg-blue-50 text-[#3B82F6]">
                                ...
                                <a href="{{ route('properties.show', $reservation->property) }}"
                                    class="absolute inset-0 bg-black/0 hover:bg-black/5 transition-colors"></a>
                            </div>
                            ...
                            <div class="hidden md:flex flex-col gap-3 h-full">
                                ...
                                <div
                                    class="flex-1 rounded-2xl bg-gray-50/70 border border-gray-150 p-4 flex flex-col justify-center">
                                    ...
                                    <span class="text-[20px] font-black text-gray-900 mt-0.5">
                                        @if($reservation->unit?->rental_fee)
                                            ₱{{ number_format($reservation->unit->rental_fee) }}<span
                                                class="text-[12px] font-normal text-gray-400">/mo</span>
                                        @else
                                ...
                                <div
                                    ...
                                    <span class="text-[13.5px] font-semibold text-gray-700 mt-1 flex items-center gap-1.5">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                            stroke="currentColor" class="w-3.5 h-3.5 text-gray-400">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div
                            class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-gray-150 pb-5 mb-5">
                            <div>
                                <a href="{{ route('properties.show', $reservation->property) }}"
                                    class="text-xl font-bold text-gray-900 hover:text-[#3B82F6] transition-colors tracking-tight">
                                    {{ $property->title ?? $reservation->property->title }}
                                </a>
                                <p class="text-[13px] text-gray-400 font-medium mt-1 flex items-center gap-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                        stroke="currentColor" class="w-3.5 h-3.5 flex-shrink-0 text-gray-300">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="flex items-center gap-2 self-start sm:self-auto">
                                <span
                                    class="inline-flex items-center gap-1.5 text-[12px] font-bold px-3 py-1 rounded-full border {{ $statusBadgeStyles }}">
                        ...
                        <div class="mb-6">
                            <h3 class="text-[13px] font-bold uppercase tracking-wider text-gray-400 mb-4">Application Parameters
                            </h3>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-8">
                                ...
                                <div class="flex items-center gap-3 text-[14px]">
                                    ...
                                    <span
                                        class="font-semibold text-gray-900 ml-auto sm:ml-0">{{ is_string($reservation->reservation_date) ? \Carbon\Carbon::parse($reservation->reservation_date)->format('M d, Y') : $reservation->reservation_date->format('M d, Y') }}</span>
                                </div>
                                ...
                                <div class="flex items-center gap-3 text-[14px]">
                                    ...
                                    <span
                                        class="font-semibold text-gray-900 ml-auto sm:ml-0">{{ $reservation->duration_of_stay ?? 'Not Specified' }}</span>
                                </div>
                                ...
                                <div class="flex items-center gap-3 text-[14px]">
                                    ...
                                    <span
                                        class="font-semibold text-gray-900 ml-auto sm:ml-0">{{ trim(($reservation->tenant->first_name ?? '') . ' ' . ($reservation->tenant->last_name ?? '')) }}</span>
                                </div>
                                ...
                                <div class="flex items-center gap-3 text-[14px]">
                                    ...
                                    <span
                                        class="font-semibold text-gray-900 ml-auto sm:ml-0">{{ $reservation->occupants_count ?? 1 }}
                                        Person(s)</span>
                                </div>
                                ...
                                <div class="flex items-center gap-3 text-[14px] sm:col-span-2">
                                    <svg class="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                        stroke-width="1.5">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M2.25 6.75c0 8.284 6.716 15 15 15h1.5a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143" />
                                    </svg>
                                    <span class="text-gray-500">Contact:</span>
                                    <span
                                        class="font-semibold text-gray-900 ml-auto sm:ml-0">{{ $reservation->tenant->contact_number ?? $reservation->tenant->phone ?? 'Not provided' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($reservation->remarks)
                            <div class="text-[14px] text-gray-600 bg-gray-50 border border-gray-150 rounded-2xl px-4 py-3.5 mb-6">
                                <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1">Message from
                                    Applicant:</span>
                                "{{ $reservation->remarks }}"
                            </div>
                        @endif
                        ...
                        @if($reservation->rejection_reason)
                            <div class="text-[14px] text-red-800 bg-red-50/50 border border-red-100 rounded-2xl px-4 py-3.5 mb-6">
                                <span class="block text-[11px] font-bold text-red-500 uppercase tracking-wide mb-1">Decline
                                    Reason:</span>
                        ...
                        @if($currentStatus === 'Pending')
                            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                                <button type="button" @click="activeReservationId = {{ $resId }}; rejectModalOpen = true;"
                                ...
                                <form action="{{ route('landlord.reservations.approve', $reservation) }}" method="POST">
                                    ...
                                    @method('PATCH')
                                    <button type="submit"
                                        class="h-10 px-5 rounded-xl bg-[#3B82F6] hover:bg-[#2563EB] text-white font-bold text-[13px] transition shadow-sm">
                                        Approve
                                    </button>
                                </form>
                            </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative transform overflow-hidden rounded-2xl bg-white shadow-xl w-full max-w-md p-6 border border-gray-100"
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-[13px] text-gray-400 mt-0.5">Provide a reason to inform the tenant why this
                            application was declined.</p>
                    </div>
                    ...
                    <form :action="`/landlord/reservations/${activeReservationId}/reject`" method="POST">
                        ...
                        <div class="mb-5">
                            <textarea id="rejection_reason" name="rejection_reason" rows="4" required
                                ...
                                class="w-full border border-gray-200 rounded-xl px-4 py-3 text-[14px] text-gray-700 placeholder-gray-400 focus:outline-none focus:border-[#3B82F6] transition resize-none"></textarea>
                        </div>
