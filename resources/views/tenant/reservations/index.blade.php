...
@section('content')
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-[50px] py-8" x-data="{
        ...
        <div class="flex items-start justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-[#2A2523]">My Reservations</h1>
                <p class="text-sm text-[#9B9F98] mt-1">Track your rental inquiries and reservations.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white border border-[#9B9F98]/15 rounded-2xl p-4">
                <p class="text-[12px] font-semibold text-[#9B9F98] mb-1">Total</p>
                <p class="text-2xl font-bold text-[#2A2523]">{{ $counts['all'] }}</p>
                <p class="text-[11px] text-[#9B9F98] mt-1">All time</p>
            </div>
            <div class="bg-[#D7E8F3] border border-[#61B2F0]/40 rounded-2xl p-4">
                <p class="text-[12px] font-semibold text-[#2A2523] mb-1">In progress</p>
                <p class="text-2xl font-bold text-[#2A2523]">
                    {{ $counts['Inquiry'] + $counts['Under Negotiation'] + $counts['Pending Rental Agreement'] + $counts['Rental Agreement Signed'] }}
                </p>
                <p class="text-[11px] text-[#9B9F98] mt-1">Awaiting action</p>
            </div>
            <div class="bg-white border border-[#9B9F98]/15 rounded-2xl p-4">
                <p class="text-[12px] font-semibold text-[#9B9F98] mb-1">Occupied</p>
                <p class="text-2xl font-bold text-[#2A2523]">{{ $counts['Occupied'] }}</p>
                <p class="text-[11px] text-[#9B9F98] mt-1">All time</p>
            </div>
            <div class="bg-white border border-[#BD5434]/30 rounded-2xl p-4">
                <p class="text-[12px] font-semibold text-[#BD5434] mb-1">Cancelled / Rejected</p>
                <p class="text-2xl font-bold text-[#2A2523]">{{ $counts['Cancelled'] + $counts['Rejected'] }}</p>
                <p class="text-[11px] text-[#9B9F98] mt-1">All time</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-[#9B9F98]/15 rounded-2xl overflow-hidden">
            @if($reservations->isEmpty())
                <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
                    ...
                    <p class="text-[13px] text-[#9B9F98]">Your rental inquiries and reservations will show up here.</p>
                </div>
            @else
                        ...
                        @foreach($reservations as $reservation)
                            ...
                            <tr class="border-t border-[#9B9F98]/10 hover:bg-[#F0EDE8]/40 transition-colors duration-150">
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    <p class="text-[12px] text-[#9B9F98]">{{ $reservation->property->landlord->contact_number ?? '—' }}</p>
                                </td>
                                <td class="px-5 py-4">
                ...
                <template x-if="selected">
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            <span class="text-[#9B9F98]">Landlord</span>
                            <span class="font-semibold text-[#2A2523]" x-text="selected.landlord_name"></span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-[#9B9F98]">Contact</span>
                            <span class="text-[#2A2523]" x-text="selected.landlord_contact"></span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <template x-if="selected.remarks">
                            <div class="pt-2 border-t border-[#9B9F98]/15">
                                <p class="text-[#9B9F98] mb-1">Your remarks</p>
