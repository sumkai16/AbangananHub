    ...
    <div class="{{ auth()->user()->shellContainerClass() }} mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-16">
        ...
        <div class="flex flex-wrap items-center gap-1.5 text-sm text-[#5B6A8E] mb-3">
            <a href="{{ route('reservations.index') }}"
                class="hover:text-[#060D26] transition-colors duration-200">My Reservations</a>
            <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
            </svg>
            <span class="text-[#060D26] font-medium">{{ $unit->unit_label ?? 'Rent' }}</span>
        </div>
                ...
                <div class="flex items-center gap-4 min-w-0 flex-1">
                    <div class="w-14 h-14 rounded-xl bg-[#F7F8FC] overflow-hidden shrink-0 ring-1 ring-[#5B6A8E]/10">
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center">
                                <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#5B6A8E" stroke-width="1.5">
                    ...
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <h1 class="text-xl font-normal text-[#060D26] truncate">{{ $unit->unit_label ?? 'Your rent' }}</h1>
                        ...
                        <p class="text-[12.5px] text-[#5B6A8E] mt-1">{{ $reservation->property->title ?? '' }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 shrink-0">
                    @if($canReview)
                        <a href="{{ route('properties.show', $reservation->property) }}#reviews"
                            class="h-11 px-4 inline-flex items-center gap-2 rounded-full bg-[#060D26] text-white text-sm font-semibold hover:brightness-95 transition-all duration-200 cursor-pointer">
                    ...
                    @if($reservation->conversation)
                        <a href="{{ route('conversations.show', $reservation->conversation) }}"
                            class="h-11 px-4 inline-flex items-center gap-2 rounded-full border border-[#E2E4EC] bg-white text-[#060D26] text-sm font-semibold hover:bg-[#F7F8FC] transition-all duration-200 cursor-pointer">
                            <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    @php
                        $tiles = [
                            ...
                            ['label' => 'Balance',  'value' => $summary['outstanding'],   'tone' => 'text-[#060D26]', 'sub' => 'Unpaid rent to date'],
                    ...
                    @foreach($tiles as $tile)
                        <x-card class="!p-4">
                            <p class="text-[11px] font-bold text-[#5B6A8E] uppercase tracking-wide mb-2">{{ $tile['label'] }}</p>
                            <p class="text-2xl font-extrabold {{ $tile['tone'] }}">
                                ₱{{ number_format($tile['value'], 2) }}
                            </p>
                            <p class="text-[11px] text-[#5B6A8E] mt-1">{{ $tile['sub'] }}</p>
                        </x-card>
                    @endforeach
                </div>
                    ...
                    <x-card class="!bg-[#ECEEF6] !border-[#C9A84C]/25">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="text-2xl font-extrabold text-[#060D26]">
                                    ₱{{ number_format(max(0, $payablePeriod['balance']), 2) }}
                                </p>
                                <p class="text-[12px] text-[#5B6A8E] mt-1">
                            ...
                            <form method="POST" action="{{ route('payments.rentCheckout', $reservation) }}" class="shrink-0">
                                @csrf
                                <button type="submit"
                                    class="h-11 px-6 w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full bg-[#060D26] text-white text-sm font-semibold hover:brightness-95 transition-all duration-200 cursor-pointer">
                        ...
                        <div>
                            ...
                            @if($summary['prepaidThrough'])
                                <p class="text-[12px] font-medium text-[#060D26] mt-1">
                                    You are paid up through {{ $summary['prepaidThrough']->format('M Y') }}.
                                </p>
                            @endif
                        </div>
                    </div>
                            ...
                            @foreach($periods as $period)
                                ...
                                <div class="px-5 py-4">
                                    ...
                                    <div class="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-2.5 text-[12.5px]">
                                        <p class="text-[#5B6A8E]">Due on <span class="text-[#060D26] font-medium">{{ $period['due_on']->format('M d, Y') }}</span></p>
                                        <p class="text-[#5B6A8E] text-right">Expected <span class="text-[#060D26] font-medium">₱{{ number_format($period['expected'], 2) }}</span></p>
                                        <p class="text-[#5B6A8E]">Paid <span class="text-[#060D26] font-medium">₱{{ number_format($period['paid'], 2) }}</span></p>
                                        {{-- Neutral for a month that hasn't arrived: not owed yet. --}}
                                        <p class="text-[#5B6A8E] text-right">Balance <span class="font-medium {{ $period['balance'] > 0 && ! $period['is_future'] ? 'text-[#DC2626]' : 'text-[#060D26]' }}">₱{{ number_format(max(0, $period['balance']), 2) }}</span></p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                ...
                @if($otherCharges->isNotEmpty())
                    <x-card flush>
                        <div class="px-5 sm:px-6 py-4 border-b border-[#E2E4EC]">
                            <h2 class="text-[15px] font-normal text-[#060D26]">Deposits &amp; other payments</h2>
                            <p class="text-[12px] text-[#5B6A8E] mt-0.5">Money outside the monthly rent cycle.</p>
                        </div>
                        {{-- Mobile card list — same data as the table below, stacked for a phone screen --}}
                        <div class="lg:hidden divide-y divide-[#E2E4EC]">
                            @foreach($otherCharges as $charge)
                                <div class="px-5 py-4">
                                    <div class="flex items-center justify-between gap-3">
                                        <p class="text-[13px] font-semibold text-[#060D26]">{{ $charge->payment_type }}</p>
                                        <p class="text-[13px] font-semibold text-[#060D26]">₱{{ number_format((float) $charge->amount, 2) }}</p>
                                    </div>
                                    <div class="flex items-center justify-between gap-3 mt-1 text-[12px] text-[#5B6A8E]">
                            ...
                            <table class="w-full min-w-[600px]">
                                <thead class="bg-[#F7F8FC] border-b border-[#E2E4EC]">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3 text-left text-[11px] font-bold text-[#5B6A8E] uppercase tracking-wide">Type</th>
                                        ...
                                        <th scope="col" class="px-5 sm:px-6 py-3 text-left text-[11px] font-bold text-[#5B6A8E] uppercase tracking-wide">Source</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-[#E2E4EC]">
                                    @foreach($otherCharges as $charge)
                                        <tr class="hover:bg-[#F7F8FC] transition-colors duration-150">
                                            ...
                                            <td class="px-4 py-3.5 text-[13px] text-[#5B6A8E]">{{ $charge->payment_type }}</td>
                                            <td class="px-4 py-3.5 text-[13px] text-[#5B6A8E]">{{ $charge->payment_method }}</td>
                                            <td class="px-4 py-3.5 text-[13px] font-semibold text-[#060D26] text-right whitespace-nowrap">
            ...
            <div class="space-y-5">
                <x-card>
                    ...
                    <dl class="space-y-2.5 text-[13px]">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <dt class="text-[#5B6A8E]">{{ $isActive ? 'Move-out' : 'Moved out' }}</dt>
                            <dd class="font-semibold text-[#060D26]">
                ...
                @unless($isActive)
                    <x-card class="bg-[#F7F8FC]">
                        ...
                        <p class="text-[12.5px] text-[#5B6A8E] leading-relaxed">
                            This tenancy ended on
                            <strong class="text-[#060D26]">{{ optional($reservation->target_move_out_date)->format('M d, Y') ?? 'an unrecorded date' }}</strong>.
