...
@section('content')
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-14 min-h-[calc(100vh-72px)]" x-data="{ bio: {{ strlen(old('bio', $user->bio ?? '')) }} }">
        ...
        <a href="{{ route('tenant.profile.show') }}"
            class="inline-flex items-center gap-1.5 text-sm font-semibold text-[#060D26] hover:brightness-90 transition-all mb-5">
        ...
        <div class="flex items-center gap-3.5 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-normal tracking-tight text-[#060D26]">Edit profile</h1>
                <p class="text-sm text-[#5B6A8E] mt-0.5">This is how landlords will see you when you inquire or reserve.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-[240px_1fr]">
            ...
            <aside class="lg:sticky lg:top-24 h-fit flex flex-col gap-4">
                <x-card flush class="p-6 text-center">
                    ...
                    <p class="text-[15px] font-bold text-[#060D26]">{{ $user->first_name }} {{ $user->last_name }}</p>
                    <p class="text-[12px] text-[#5B6A8E] mt-1 truncate">{{ $user->email }}</p>

                    <div class="flex justify-center gap-1.5 mt-3">
                        <span class="bg-[#ECEEF6] text-[#060D26] text-[11px] font-bold px-2.5 py-1 rounded-full">Tenant</span>
                        @if($user->hasRole('Landlord'))
                            <span class="bg-[#22C55E]/[0.07] text-[#15803D] text-[11px] font-bold px-2.5 py-1 rounded-full">Landlord</span>
                        @endif
                    </div>

                    <div class="border-t border-[#E2E4EC] mt-4 pt-4 grid grid-cols-2 gap-2 text-center">
                        <div>
                            <p class="text-[18px] font-extrabold text-[#060D26]">{{ $user->reviews()->count() }}</p>
                            <p class="text-[11px] text-[#5B6A8E] mt-0.5">{{ Str::plural('Review', $user->reviews()->count()) }}</p>
                        </div>
                        <div>
                            <p class="text-[18px] font-extrabold text-[#060D26]">{{ $user->favorites()->count() }}</p>
                            <p class="text-[11px] text-[#5B6A8E] mt-0.5">Saved</p>
                        </div>
                    </div>
                </x-card>

                {{-- Privacy note --}}
                <div class="rounded-2xl bg-[#ECEEF6] p-5">
                    <div class="w-8 h-8 rounded-lg bg-white flex items-center justify-center mb-3 shadow-sm">
                        <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="#060D26" stroke-width="1.8">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                        </svg>
                    </div>
                    <p class="text-[12.5px] font-semibold text-[#060D26] leading-snug">Your email stays private</p>
                    <p class="text-[11.5px] text-[#060D26]/80 leading-relaxed mt-1.5">
            ...
            <div>
                <form action="{{ route('tenant.profile.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <x-card flush class="p-6 sm:p-8 mb-5">
                        <div class="flex items-center gap-3 mb-5">
                            <div class="w-10 h-10 rounded-xl bg-[#ECEEF6] flex items-center justify-center shrink-0">
                                <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="#060D26" stroke-width="1.8">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6.827 6.175A2.31 2.31 0 015.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 00-1.134-.175 2.31 2.31 0 01-1.64-1.055l-.822-1.316a2.192 2.192 0 00-1.736-1.039 48.774 48.774 0 00-5.232 0 2.192 2.192 0 00-1.736 1.039l-.821 1.316z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 12.75a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0z" />
                                </svg>
                            </div>
                            <h2 class="text-[15px] font-normal text-[#060D26]">Profile picture</h2>
                        </div>
                        <div class="flex items-center gap-5">
                            <div id="avatar-preview-wrapper">
                                ...
                                @else
                                    <div id="avatar-preview" class="w-16 h-16 rounded-2xl bg-[#060D26] flex items-center justify-center text-white text-[22px] font-bold">
                                        {{ strtoupper(substr($user->first_name, 0, 1)) }}{{ strtoupper(substr($user->last_name, 0, 1)) }}
                                    </div>
                                @endif
                            </div>
                            <div>
                                <label for="profile_picture"
                                    class="inline-flex items-center gap-2 px-4 py-2 border border-[#E2E4EC] rounded-lg text-[13px] font-semibold text-[#060D26] hover:brightness-95 bg-white transition-all cursor-pointer">
                                ...
                                <p class="text-[11px] text-[#5B6A8E] mt-1.5">JPG, PNG. Max 2MB. Cropped to square.</p>
                    ...
                    <x-card flush class="p-6 sm:p-8 mb-5">
                        ...
                        <div class="mb-5">
                            <label for="contact_number" class="mb-1.5 block text-xs font-semibold uppercase tracking-widest text-[#5B6A8E]">Contact number</label>
                        ...
                        <div>
                            <label for="bio" class="mb-1.5 block text-xs font-semibold uppercase tracking-widest text-[#5B6A8E]">About me</label>
                    ...
                    <x-card flush class="p-6 sm:p-8 mb-6">
                        <div class="flex items-center gap-3 mb-5">
                            ...
                            <h2 class="text-[15px] font-normal text-[#060D26]">Account</h2>
                        </div>
                        <label for="email" class="mb-1.5 block text-xs font-semibold uppercase tracking-widest text-[#5B6A8E]">Email address</label>
                        <input type="email" id="email" value="{{ $user->email }}" disabled
                            class="h-10 w-full rounded-xl border border-[#E2E4EC] bg-[#F7F8FC] px-3.5 text-sm text-[#5B6A8E] cursor-not-allowed">
                        <p class="text-[11px] text-[#5B6A8E] mt-1.5">Email can be changed in <a href="{{ route('profile.edit') }}" class="text-[#060D26] font-semibold hover:underline">Account Settings</a>.</p>
                    </x-card>
                    ...
                    <div class="flex items-center justify-end gap-3">
                        ...
                        <button type="submit"
                            class="px-6 py-2.5 bg-[#DA8E77] text-[#060D26] rounded-lg text-[13px] font-semibold hover:bg-[#C97A61] transition-all shadow-sm active:scale-[0.98]">Save changes</button>
                    </div>
                </form>
            </div>
