...
@php
    $statusBadge = [
        ...
        'Under Negotiation'        => 'bg-blue-50 text-blue-700 border-blue-200',
        'Pending Rental Agreement' => 'bg-amber-50 text-amber-700 border-amber-200',
        'Rental Agreement Signed'  => 'bg-indigo-50 text-indigo-700 border-indigo-200',
    ...
    $statusDot = [
        'Inquiry'                  => 'bg-gray-400',
        'Under Negotiation'        => 'bg-blue-400',
        'Pending Rental Agreement' => 'bg-amber-400',
        'Rental Agreement Signed'  => 'bg-indigo-400',
...
<div class="max-w-5xl" x-data="{ cancelOpen: false, rejectOpen: false }">
        ...
        @if(!$isTerminal)
            <div class="flex items-center gap-2">
                <button @click="rejectOpen = true"
                    class="inline-flex items-center gap-1.5 h-9 px-4 text-[13px] font-semibold border border-red-200 text-red-600 rounded-xl hover:bg-red-50 transition-colors">
                ...
                <button @click="cancelOpen = true"
                    class="inline-flex items-center gap-1.5 h-9 px-4 text-[13px] font-semibold border border-amber-200 text-amber-700 rounded-xl hover:bg-amber-50 transition-colors">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-2.5 mb-1">
                    ...
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border {{ $statusBadge[$rs] ?? '' }}">
                        <span class="w-1.5 h-1.5 rounded-full {{ $statusDot[$rs] ?? 'bg-gray-300' }}"></span>
                        {{ $rs }}
                    </span>
                </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-3">
                    ...
                    <h2 class="text-[14px] font-bold text-[#1A1A2E]">Reservation Details</h2>
                </div>
                <div class="divide-y divide-gray-50">
                    @foreach([
                        ...
                        <div class="px-6 py-3.5 flex items-center justify-between">
                            <p class="text-[12px] text-gray-400 font-medium">{{ $label }}</p>
                            <p class="text-[13.5px] font-semibold text-[#1A1A2E]">{{ $value }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            @if($reservation->agreed_at)
                <div class="bg-indigo-50 border border-indigo-100 rounded-3xl shadow-sm p-6">
                    <p class="text-[10px] font-bold uppercase tracking-widest text-indigo-500 mb-3">Agreement Signed</p>
                    <div class="space-y-2">
                        <div class="flex justify-between text-[13px]">
                            <span class="text-indigo-500">Signed at</span>
                            <span class="font-semibold text-indigo-800">{{ $reservation->agreed_at->format('F j, Y \a\t g:i A') }}</span>
                        </div>
                        <div class="flex justify-between text-[13px]">
                            <span class="text-indigo-500">IP Address</span>
                            <span class="font-semibold text-indigo-800 font-mono">{{ $reservation->agreed_ip ?? '—' }}</span>
                        </div>
                    </div>
                    @if($reservation->agreement_terms_notes)
                        <div class="mt-4 pt-4 border-t border-indigo-200/60">
                            <p class="text-[10px] font-bold uppercase tracking-widest text-indigo-400 mb-1.5">Negotiated Terms</p>
                            <p class="text-[13px] text-indigo-800 leading-relaxed whitespace-pre-wrap">{{ $reservation->agreement_terms_notes }}</p>
                        </div>
                    @endif
                </div>
            @endif
            ...
            @if($reservation->rejection_reason)
                <div class="bg-red-50 border border-red-100 rounded-3xl shadow-sm p-6">
                    <p class="text-[10px] font-bold uppercase tracking-widest text-red-400 mb-2">Rejection Reason</p>
                    <p class="text-[13.5px] text-red-800 leading-relaxed">{{ $reservation->rejection_reason }}</p>
                </div>
            @endif
            ...
            @if($reservation->payments->count())
                <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-3">
                        ...
                        <h2 class="text-[14px] font-bold text-[#1A1A2E]">Payments</h2>
                    </div>
                    <div class="divide-y divide-gray-50">
                        @foreach($reservation->payments as $payment)
                            <div class="px-6 py-4 flex items-center justify-between gap-4">
                                <div>
                                    <p class="text-[13.5px] font-semibold text-[#1A1A2E]">{{ $payment->payment_type }} Payment</p>
                                ...
                                <div class="text-right shrink-0">
                                    <p class="text-[14px] font-extrabold text-[#1A1A2E]">₱{{ number_format($payment->amount, 2) }}</p>
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    ...
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <p class="text-[13.5px] font-bold text-[#1A1A2E]">{{ trim(($tenant->first_name ?? '') . ' ' . ($tenant->last_name ?? '')) ?: '—' }}</p>
                    ...
                    <div class="space-y-2 text-[12.5px]">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-[#1A1A2E]">{{ $tenant->contact_number ?? '—' }}</span>
                        </div>
                    </div>
                    @if($tenant)
            ...
            <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    ...
                    @endif
                    <p class="text-[13.5px] font-bold text-[#1A1A2E]">{{ $property->title ?? '—' }}</p>
                    <p class="text-[12px] text-gray-400 mt-0.5">{{ $property->address ?? '—' }}</p>
                    @if($reservation->unit)
                        <div class="mt-3 pt-3 border-t border-gray-50 space-y-1.5 text-[12.5px]">
                            <div class="flex justify-between">
                                <span class="text-gray-400">Unit</span>
                                <span class="font-semibold text-[#1A1A2E]">{{ $reservation->unit->unit_label }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-400">Rental Fee</span>
                                <span class="font-semibold text-[#1A1A2E]">₱{{ number_format($reservation->unit->rental_fee, 2) }}/mo</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-400">Unit Status</span>
                                <span class="font-semibold text-[#1A1A2E]">{{ $reservation->unit->availability_status }}</span>
                            </div>
                        </div>
                    @endif
                </div>
                ...
                <div class="p-5">
                    ...
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0">
                            <span class="text-indigo-600 text-[12px] font-extrabold">
                                {{ strtoupper(substr($landlord->first_name ?? '?', 0, 1)) }}{{ strtoupper(substr($landlord->last_name ?? '', 0, 1)) }}
                            </span>
                        </div>
                        <div>
                            <p class="text-[13px] font-bold text-[#1A1A2E]">{{ trim(($landlord->first_name ?? '') . ' ' . ($landlord->last_name ?? '')) ?: '—' }}</p>
                            <p class="text-[11.5px] text-gray-400">{{ $landlord->email ?? '—' }}</p>
                        </div>
                    </div>
                    @if($landlord)
    ...
    <div x-show="cancelOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-3xl shadow-xl max-w-md w-full p-7 z-10">
            <div class="w-12 h-12 rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-center mx-auto mb-4">
            ...
            <h3 class="text-[16px] font-extrabold text-[#1A1A2E] text-center mb-1">Force Cancel Reservation?</h3>
            <p class="text-[13px] text-gray-400 text-center mb-5">
            ...
            <form action="{{ route('admin.reservations.forceCancel', $reservation) }}" method="POST">
                @csrf @method('PATCH')
                <div class="mb-4">
                    <label class="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">Admin Note <span class="font-normal normal-case">(optional)</span></label>
                    <textarea name="admin_note" rows="2" placeholder="Reason for cancellation…"
                        class="w-full px-3.5 py-2.5 text-[13px] rounded-xl border border-gray-200 bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-amber-300/40 focus:border-amber-400 resize-none transition-all"></textarea>
                </div>
    ...
    <div x-show="rejectOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-3xl shadow-xl max-w-md w-full p-7 z-10">
            ...
            <h3 class="text-[16px] font-extrabold text-[#1A1A2E] text-center mb-1">Force Reject Reservation?</h3>
            <p class="text-[13px] text-gray-400 text-center mb-5">
            ...
            <form action="{{ route('admin.reservations.forceReject', $reservation) }}" method="POST">
                @csrf @method('PATCH')
                <div class="mb-4">
                    <label class="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">Rejection Reason <span class="font-normal normal-case">(optional)</span></label>
                    <textarea name="admin_note" rows="2" placeholder="Reason for rejection…"
                        class="w-full px-3.5 py-2.5 text-[13px] rounded-xl border border-gray-200 bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-red-300/40 focus:border-red-400 resize-none transition-all"></textarea>
                </div>
                <div class="flex gap-3">
                    ...
                    <button type="submit"
                        class="flex-1 h-10 text-[13.5px] font-bold bg-red-500 text-white rounded-xl hover:bg-red-600 transition-colors">
                        Reject Reservation
                    </button>
                </div>
            </form>
