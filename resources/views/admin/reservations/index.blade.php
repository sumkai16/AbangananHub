...
@php
    $allStatuses = [
        'Inquiry'                  => ['label' => 'Inquiry',           'dot' => 'bg-gray-400'],
        'Under Negotiation'        => ['label' => 'Negotiation',       'dot' => 'bg-blue-400'],
        'Pending Rental Agreement' => ['label' => 'Pending Agreement', 'dot' => 'bg-amber-400'],
        'Rental Agreement Signed'  => ['label' => 'Agreement Signed',  'dot' => 'bg-indigo-400'],
        'Occupied'                 => ['label' => 'Occupied',          'dot' => 'bg-emerald-400'],
...
<div class="max-w-[1400px]">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 shadow-sm">
            <p class="text-[11px] font-bold uppercase tracking-widest text-emerald-600 mb-1">Occupied</p>
            <p class="text-[28px] font-extrabold text-emerald-700 leading-none">{{ number_format($counts['Occupied']) }}</p>
            <p class="text-[11px] text-emerald-600 mt-1">Units currently rented</p>
        </div>
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 shadow-sm">
            <p class="text-[11px] font-bold uppercase tracking-widest text-blue-600 mb-1">In Progress</p>
            <p class="text-[28px] font-extrabold text-blue-700 leading-none">
                {{ number_format($counts['Inquiry'] + $counts['Under Negotiation'] + $counts['Pending Rental Agreement'] + $counts['Rental Agreement Signed']) }}
            </p>
            <p class="text-[11px] text-blue-600 mt-1">Active pipeline</p>
        </div>
        <div class="bg-red-50 border border-red-100 rounded-2xl p-4 shadow-sm">
            <p class="text-[11px] font-bold uppercase tracking-widest text-red-500 mb-1">Closed</p>
            <p class="text-[28px] font-extrabold text-red-600 leading-none">{{ number_format($counts['Cancelled'] + $counts['Rejected']) }}</p>
            <p class="text-[11px] text-red-500 mt-1">Cancelled + Rejected</p>
        </div>
    </div>
    ...
    @if($reservations->isEmpty())
        <div class="bg-white border border-gray-100 rounded-3xl p-16 text-center shadow-sm">
            ...
            <p class="text-[15px] font-bold text-[#1A1A2E]">No reservations found</p>
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-gray-50/60 border-b border-gray-100">
                            ...
                            <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Move-In</th>
                        ...
                        @foreach($reservations as $res)
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-[12px] text-gray-400">{{ $res->tenant->email ?? '—' }}</p>
                                </td>
                                ...
                                {{-- Status --}}
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border {{ $statusBadge[$res->rental_status] ?? 'bg-gray-50 text-gray-500 border-gray-200' }}">
                                        <span class="w-1.5 h-1.5 rounded-full {{ $allStatuses[$res->rental_status]['dot'] ?? 'bg-gray-300' }}"></span>
                                        {{ $res->rental_status }}
                                    </span>
                                </td>
