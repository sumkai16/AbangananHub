...
<div class="max-w-7xl space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex items-center gap-2 bg-white border border-gray-100 rounded-2xl px-4 py-2.5 shadow-sm text-[13px] text-gray-500">
        ...
        @foreach($statCards as $card)
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between mb-3">
                    <p class="text-[11.5px] font-bold text-gray-400 uppercase tracking-wider leading-tight">{{ $card['label'] }}</p>
                    <div class="w-8 h-8 rounded-xl {{ $c['icon_bg'] }} flex items-center justify-center shrink-0">
                        <svg class="w-4 h-4 {{ $c['icon_text'] }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                            <path stroke-linecap="round" stroke-linejoin="round" d="{{ $card['icon'] }}" />
                        </svg>
                    </div>
                </div>
                <p class="text-[28px] font-extrabold {{ $c['val_text'] }} leading-none">{{ number_format($card['value']) }}</p>
        ...
        <div class="lg:col-span-7 bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
            <div class="flex items-center justify-between mb-5">
                ...
                <div class="flex items-center gap-4 text-[12px] text-gray-400">
                    <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 rounded-full bg-[#286CD2] inline-block"></span>Users</span>
                    <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 rounded-full bg-emerald-500 inline-block"></span>Properties</span>
                    <span class="flex items-center gap-1.5"><span class="w-3 h-0.5 rounded-full bg-amber-400 inline-block"></span>Reservations</span>
                </div>
            </div>
            <div class="h-56">
                    ...
                    @foreach($distItems as $item)
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                <span class="text-[13px] font-bold text-[#1A1A2E]">{{ number_format($item['value']) }}</span>
                ...
                <div class="flex gap-1 bg-gray-50 rounded-xl p-1">
                    <button @click="tab = 'landlords'"
                        ...
                        @if($pendingVerifications > 0)
                            <span class="ml-1 bg-amber-100 text-amber-700 text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $pendingVerifications }}</span>
                        @endif
                    </button>
                    <button @click="tab = 'properties'"
                        ...
                        @if($pendingListings > 0)
                            <span class="ml-1 bg-amber-100 text-amber-700 text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $pendingListings }}</span>
                        @endif
                    </button>
                    <button @click="tab = 'units'"
                        ...
                        Units
                        @if($pendingUnits > 0)
                            <span class="ml-1 bg-amber-100 text-amber-700 text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $pendingUnits }}</span>
                        @endif
                    </button>
                </div>
            </div>
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="bg-gray-50/50 border-b border-gray-50">
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Name</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Business</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Date Submitted</th>
                            ...
                            @foreach($pendingVerificationList as $v)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-3.5">
                                        <p class="text-[13.5px] font-semibold text-[#1A1A2E]">{{ $v->user->first_name }} {{ $v->user->last_name }}</p>
                                    ...
                                    <td class="px-6 py-3.5">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-amber-50 text-amber-700 border border-amber-200">Pending</span>
                                    </td>
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="bg-gray-50/50 border-b border-gray-50">
                                ...
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Type</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($pendingListingList as $p)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-3.5">
                                        <p class="text-[13.5px] font-semibold text-[#1A1A2E] truncate max-w-[180px]">{{ $p->title ?? 'Untitled' }}</p>
                                    </td>
                                    <td class="px-6 py-3.5">
                                        <span class="text-[11.5px] font-bold bg-[#286CD2]/10 text-[#286CD2] px-2 py-0.5 rounded-full">{{ $p->property_type ?? '—' }}</span>
                                    </td>
                                    <td class="px-6 py-3.5 text-[13px] text-gray-600">{{ $p->landlord?->first_name }} {{ $p->landlord?->last_name }}</td>
                                    <td class="px-6 py-3.5 text-[13px] text-gray-400">
            ...
            <div x-show="tab === 'units'" x-cloak>
                ...
                @else
                    <table class="min-w-full">
                        <thead>
                            <tr class="bg-gray-50/50 border-b border-gray-50">
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Unit</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Property</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Date Submitted</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Status</th>
                                <th class="px-6 py-3"></th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($pendingUnitList as $u)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-3.5 text-[13.5px] font-semibold text-[#1A1A2E]">{{ $u->unit_label ?? '—' }}</td>
                                    <td class="px-6 py-3.5 text-[13px] text-gray-600 truncate max-w-[180px]">{{ $u->property?->title ?? '—' }}</td>
                                    <td class="px-6 py-3.5 text-[13px] text-gray-400">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                @foreach($resOverview as $item)
                    <div class="rounded-2xl border p-4 text-center {{ $item['cls'] }}">
                        ...
                        <p class="text-[11.5px] font-bold mt-1.5 uppercase tracking-wide opacity-80">{{ $item['label'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                @foreach($quickActions as $action)
                    <a href="{{ $action['href'] }}"
                        ...
                        <div class="w-9 h-9 rounded-xl {{ $action['color'] }} flex items-center justify-center shrink-0 shadow-sm">
                            <svg class="w-4.5 h-4.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $action['icon'] }}" />
                            </svg>
                        </div>
                        <div class="min-w-0">
                            <p class="text-[13px] font-bold text-[#1A1A2E] truncate group-hover:text-[#286CD2] transition-colors">{{ $action['label'] }}</p>
