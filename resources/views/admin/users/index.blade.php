    ...
    <div class="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6 items-start">
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white/70 backdrop-blur-xl border border-white/30 rounded-2xl p-5 shadow-lg">
                <h2 class="text-[13.5px] font-bold text-[#1A1A2E] mb-4">User Breakdown</h2>
                <div class="relative h-40 flex items-center justify-center mb-4">
                    <canvas id="roleBreakdownChart"></canvas>
                    <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <p class="text-[24px] font-extrabold text-[#1A1A2E] leading-none">{{ number_format($users->total()) }}</p>
                        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mt-0.5">Users</p>
                    </div>
                </div>
                <div class="space-y-2">
                    @php
                    ...
                    @foreach ($roleCounts as $label => $count)
                        <div class="flex items-center justify-between text-[12.5px]">
                            ...
                            <span class="font-bold text-[#1A1A2E]">{{ number_format($count) }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            {{-- Account status --}}
            <div class="bg-white/70 backdrop-blur-xl border border-white/30 rounded-2xl p-5 shadow-lg">
                <h2 class="text-[13.5px] font-bold text-[#1A1A2E] mb-4">Account Status</h2>
                <div class="space-y-3">
                    @php
                        $statusMeta = [
                            'active'    => ['label' => 'Active',    'color' => 'bg-emerald-500'],
                            'suspended' => ['label' => 'Suspended', 'color' => 'bg-red-400'],
                            'inactive'  => ['label' => 'Inactive',  'color' => 'bg-gray-300'],
                        ];
                    ...
                    @foreach ($statusCounts as $key => $count)
                        <div>
                            <div class="flex items-center justify-between text-[12.5px] mb-1">
                                <span class="text-gray-600">{{ $statusMeta[$key]['label'] }}</span>
                                <span class="font-bold text-[#1A1A2E]">{{ number_format($count) }}</span>
                            </div>
