    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2" x-data="{ deleteOpen: false }">
            ...
            <form action="{{ route('admin.users.updateStatus', $user) }}" method="POST">
                ...
                <button type="submit"
                    class="inline-flex items-center gap-1.5 h-9 px-4 text-[13px] font-semibold rounded-xl transition-colors {{ $isActive ? 'border border-amber-200 text-amber-700 hover:bg-amber-50' : 'border border-emerald-200 text-emerald-700 hover:bg-emerald-50' }}">
                    @if($isActive)
            ...
            @if($user->user_id !== auth()->id())
                ...
                <div x-show="deleteOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-3xl shadow-xl max-w-sm w-full p-7 z-10">
                        ...
                        <h3 class="text-[16px] font-extrabold text-[#1A1A2E] text-center mb-1">Delete User?</h3>
                        <p class="text-[13px] text-gray-400 text-center mb-6">
                            This will permanently remove <strong class="text-[#1A1A2E]">{{ trim($user->first_name . ' ' . $user->last_name) }}</strong> and all their data. This cannot be undone.
                        </p>
