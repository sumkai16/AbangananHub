...
<div class="max-w-[1600px] mx-auto">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
            <h1 class="text-2xl font-normal text-[#060D26] tracking-tight">Reviews</h1>
            <p class="text-[13.5px] text-[#5B6A8E] mt-1">Moderate tenant reviews. Flag inappropriate or fake content.</p>
        </div>
        <span class="text-[13px] font-semibold text-[#5B6A8E]">{{ number_format($counts['all']) }} total</span>
    </div>

    {{-- Stat cards --}}
    <div class="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-6">
        @foreach([5, 4, 3, 2, 1] as $stars)
            <div class="bg-white border border-[#E2E4EC] rounded-2xl p-4 shadow-[0_1px_3px_rgba(6,13,38,0.06)] text-center">
                <div class="flex items-center justify-center gap-0.5 mb-1">
                    @for($i = 1; $i <= 5; $i++)
                        <svg class="w-3 h-3" fill="{{ $i <= $stars ? '#FBBF24' : '#E2E4EC' }}" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                        </svg>
                    @endfor
                </div>
                <p class="text-[20px] font-extrabold text-[#060D26]">{{ $counts[$stars] }}</p>
                <p class="text-[10px] text-[#5B6A8E]">{{ $stars }}-star</p>
            </div>
        @endforeach
    </div>
    ...
    @if($reviews->isEmpty())
        <div class="bg-white border border-[#E2E4EC] rounded-2xl p-16 text-center shadow-[0_1px_3px_rgba(6,13,38,0.06)]">
            ...
            <p class="text-[15px] font-bold text-[#060D26]">No reviews found</p>
            <p class="text-[13px] text-[#5B6A8E] mt-1">{{ $search ? 'Try adjusting your search.' : 'No reviews yet.' }}</p>
        </div>
    @else
        <div class="space-y-3">
            @foreach($reviews as $review)
                <x-card class="flex flex-col sm:flex-row gap-4 sm:gap-5 {{ $review->is_hidden ? 'opacity-60' : '' }}"
                    ...
                    <div class="shrink-0 text-center w-14">
                        <p class="text-[28px] font-extrabold text-[#060D26] leading-none">{{ $review->rating }}</p>
                        <div class="flex justify-center gap-0.5 mt-1">
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="w-2.5 h-2.5" fill="{{ $i <= $review->rating ? '#FBBF24' : '#E2E4EC' }}" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                </svg>
                            @endfor
                        </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-start justify-between gap-3 mb-2">
                            <div>
                                <p class="text-[13.5px] font-bold text-[#060D26]">
                                    {{ trim(($review->tenant->first_name ?? '') . ' ' . ($review->tenant->last_name ?? '')) ?: '—' }}
                                    <span class="text-[#5B6A8E] font-normal">reviewed</span>
                                    {{ $review->property->title ?? '—' }}
                                </p>
                                <p class="text-[11.5px] text-[#5B6A8E] mt-0.5">
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                @if($review->is_hidden)
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-[#FBBF24]/15 text-[#060D26] text-[11px] font-semibold rounded-full">
                                ...
                                <button @click="confirmOpen = true"
                                    class="inline-flex items-center gap-1.5 h-8 px-3 text-[12px] font-semibold border rounded-xl transition-colors shrink-0
                                        {{ $review->is_hidden
                                            ...
                                            : 'border-[#FBBF24]/50 text-[#060D26] hover:bg-[#FBBF24]/5' }}">
                                    @if($review->is_hidden)
                        ...
                        @if($review->review_comment)
                            <p class="text-[13px] text-[#060D26] leading-relaxed">{{ $review->review_comment }}</p>
                        @else
                            <p class="text-[13px] text-[#5B6A8E] italic">No comment.</p>
                        @endif
                        ...
                        @if($review->landlord_reply)
                            <div class="mt-3 pl-4 border-l-2 border-[#C9A84C]/30">
                                ...
                                @if($review->landlord_replied_at)
                                    <p class="text-[11px] text-[#5B6A8E] mt-1">{{ $review->landlord_replied_at->format('M d, Y') }}</p>
                                @endif
                            </div>
                        @endif
                    </div>
                    ...
                    <div x-show="confirmOpen" x-cloak class="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        ...
                        <div class="relative bg-white rounded-3xl shadow-xl max-w-sm w-full p-7 z-10">
                            ...
                            <h3 class="text-[16px] font-normal text-[#060D26] text-center mb-1">
                                {{ $review->is_hidden ? 'Unhide Review?' : 'Hide Review?' }}
                            </h3>
                            <p class="text-[13px] text-[#5B6A8E] text-center mb-6">
                                @if($review->is_hidden)
                                    This will make the {{ $review->rating }}-star review by
                                    <strong class="text-[#060D26]">{{ $review->tenant->first_name ?? 'this tenant' }}</strong>
                            ...
                            <div class="flex gap-3">
                                <button @click="confirmOpen = false"
                                    class="flex-1 h-10 text-[13.5px] font-semibold border border-[#E2E4EC] text-[#5B6A8E] rounded-xl hover:brightness-95 transition-colors">
                                    Cancel
                                </button>
