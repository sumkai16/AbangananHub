    ...
    <div class="max-w-[1400px]">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-[13.5px] text-gray-500 mt-1">{{ $property->title }}</p>
            </div>
        </div>

        @if($unit->isRejected() && $unit->rejection_reason)
            <div class="mb-6 px-5 py-4 rounded-2xl bg-red-50 border border-red-100 flex items-start gap-3">
                ...
                <div>
                    <p class="text-[13px] font-bold text-red-700">Previously rejected</p>
                    <p class="text-[13px] text-red-600 mt-0.5">{{ $unit->rejection_reason }}</p>
                </div>
            </div>
        @endif
            ...
            <div class="lg:col-span-8 space-y-5">
                ...
                <div class="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
                    ...
                    @if($photos->isEmpty() && !$video)
                        <p class="text-[13px] text-gray-400">No media submitted for this unit.</p>
                    @else
                        @if($photos->isNotEmpty())
                            <div class="grid grid-cols-3 sm:grid-cols-4 gap-3 mb-4">
                                @foreach($photos as $photo)
                                    <a href="{{ $photo->media_url }}" target="_blank"
                                        class="aspect-square rounded-xl overflow-hidden bg-gray-50 border border-gray-100 block">
                        ...
                        @if($video)
                            <div class="rounded-xl overflow-hidden bg-gray-50 border border-gray-100 max-w-sm">
