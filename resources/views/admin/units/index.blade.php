    ...
    <div class="max-w-[1600px] mx-auto">
        ...
        @if ($units->isEmpty())
            <div class="bg-white border border-[#E2E4EC] rounded-2xl p-16 text-center shadow-[0_1px_3px_rgba(6,13,38,0.06)]">
                ...
                <p class="text-[15px] font-bold text-[#060D26]">No units here</p>
                <p class="text-[13px] text-[#5B6A8E] mt-1">No units match this tab right now.</p>
            </div>
        @else
            <x-card flush class="divide-y divide-[#E2E4EC]">
                @foreach ($units as $unit)
                    ...
                    <a href="{{ route('admin.units.show', [$unit->property, $unit]) }}"
                        ...
                        <div class="w-11 h-11 rounded-xl bg-[#ECEEF6] overflow-hidden shrink-0 flex items-center justify-center">
                            @if($thumb)
                                <img src="{{ $thumb->media_url }}" alt="{{ $unit->unit_label }}" loading="lazy" class="w-full h-full object-cover">
                            @else
                                <svg class="w-5 h-5 text-[#8a6e1e]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        ...
                        <div class="min-w-0 flex-1 basis-48">
                            ...
                            <p class="text-[12px] text-[#5B6A8E] truncate">{{ $unit->property->title ?? '—' }}</p>
                        </div>

                        <div class="min-w-0 flex-1 basis-40">
                            <p class="text-[11px] font-semibold uppercase tracking-wider text-[#94A3B8]">Landlord</p>
                            <p class="text-[13px] text-[#060D26] font-medium truncate">
                        ...
                        <div class="shrink-0 w-24">
                            ...
                            <p class="text-[13px] text-[#060D26] font-medium">₱{{ number_format($unit->rental_fee, 2) }}</p>
                        </div>
