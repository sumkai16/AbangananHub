                        ...
                        @elseif ($property->publication_status === 'Suspended')
                            <div class="flex sm:flex-col gap-2 sm:items-stretch shrink-0 sm:w-[130px]">
                                <form method="POST" action="{{ route('admin.listings.unsuspend', $property->property_id) }}"
                                    ...
                                    @csrf
                                    <button type="submit"
                                        class="w-full h-10 inline-flex items-center justify-center gap-2 rounded-xl bg-[#DA8E77] hover:bg-[#C97A61] text-[#060D26] text-[13px] font-bold transition-all duration-200 shadow-[0_1px_2px_rgba(6,13,38,0.35)] cursor-pointer">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                        </svg>
                                        Unsuspend
