...
- [x] Occupancy Monitoring (landlord tracks unit occupancy status — lives on `landlord/analytics` since Aug 30 2026; the standalone `landlord/occupancy` page was deleted as a duplicate)
- [x] Tenant Ratings (landlord rates tenants)
- [x] Report Analytics (admin CSV export, data visualization)
- [x] Landlord CSV export (Units, Reservations, Tenants — filter-aware, the Occupancy export went with that page; Analytics keeps its own)
- [x] Walk-in Tenants (landlord records an offline-arranged tenancy directly to Occupied — a lightweight non-login tenant account, no escrow; badged Walk-in everywhere as landlord-asserted, not platform-verified. An initial payment becomes required, not optional, once the move-in date is today or earlier — see ARCHITECTURE.md)
- [x] Rent Ledger (derived monthly billing periods with a five-value status — Paid/Partial/Overdue/Upcoming/Paid Ahead — for any occupied tenancy, walk-in or platform; landlord records offline payments — Cash/GCash/Bank/Maya/Check — with printable receipts. The portfolio collections view (`landlord/payments`) leads with the current billing cycle — Due This Month, Collected This Month, Total Unpaid, Overdue — sorted worst-first. A wrongly-recorded payment can be voided (never deleted) and optionally replaced in one motion; every void is audit-logged and visible to admin. See ARCHITECTURE.md)
