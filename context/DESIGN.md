...
- **Two container widths, chosen by context — always with `mx-auto`:**
  ...
  **Views that pick their layout at runtime must derive their width, not hard-code it.** Five views render in a sidebar shell for one role and the public shell for another (`conversations/index`, `conversations/show`, `tenant/reservations/index`, `profile/edit`, `landlord/profile/show`). Each hard-coded a single width, so it was wrong for one of its two audiences. They now call `auth()->user()->shellContainerClass()`, which returns the right cap for the shell; pass the view's own condition when it isn't the landlord default — `shellContainerClass($isOwner)`, `shellContainerClass(auth()->user()->hasRole('Admin'))`. `User::usesLandlordShell()` holds the landlord-not-admin condition that four `@extends` lines share, so the layout choice and the width can't drift apart.
  ...
  - **Third instance, July 26 2026:** `profile/edit` and `profile/show` had drifted off this list's own pattern — both still had `@extends(auth()->user()->hasRole('Admin') ? 'layouts.admin' : 'layouts.app', ...)`, so a Landlord opening Account Settings got the public `layouts.app` shell (no sidebar) instead of `layouts.landlord`, even though the width-derivation logic (`shellContainerClass`) had already been wired up correctly. Fixed to `auth()->user()->hasRole('Admin') ? 'layouts.admin' : (auth()->user()->usesLandlordShell() ? 'layouts.landlord' : 'layouts.app')`, matching `agreements/show`/`conversations/index`/`tenant/reservations/index`. **The width fix and the layout-name fix are two different lines that can drift independently** — fixing `shellContainerClass()` calls doesn't guarantee the `@extends(...)` line was updated to match, so check both when a role reports the wrong shell.
...
- **Editorial column** (`lg:col-span-7`): breadcrumb → `font-display` title → location + rating → price → description → 2×2 fact tiles → single CTA + favourite → landlord row, then stacked `<section>`s (Details, Amenities, House rules, Location, Reviews) with `id`s so the hero can link into them. **Amenities precedes House rules** (July 24 2026) — what the place offers is what a tenant is deciding on; the restrictions are a qualifier they read once interested, and leading with a column of red ✗ icons set a negative tone directly under the landlord row.
...
- **The h1 is the one place `font-display` earns an italic**: "Find your next *home* in Cebu", accent word in aqua `#69D2C6` — legal here because it sits on a dark ground, not white (§3 bans it as foreground on white only).
- **The signature is the white search pill's lift off the dark** (`shadow-[0_18px_50px_rgba(15,23,42,0.28)]`). Everything else in the hero is deliberately quiet so that one element carries it. The only other effect is a single blurred aqua bloom for depth.
- **Expanded, the hero is `min-h-[calc(100vh-72px)] flex items-center`** — a full viewport *below the header*, not a flat `100vh`. A `100vh` section starts 72px down and pushes its own bottom edge off screen, so the fold lands mid-hero. `min-h` rather than `h` lets a short laptop or landscape phone grow the section instead of clipping the search pill. **Accepted tradeoff:** the listing grid is entirely below the fold on arrival, and no affordance currently signals it — the category strip used to peek and no longer does.
- **The hero collapses when the page is doing work.** Full height on a clean page 1; the moment any filter, sort, or page > 1 is active it becomes a `py-7` band carrying just the search pill. A 380px hero above results you're comparing is furniture. `$heroStats === null` is the signal — the controller only computes it in the expanded case, so the view has no second condition to keep in sync.
- **The trust strip states live counts** (`13 listings live`, `22 units available now`) from `PropertyController@index`, plus one platform truth ("Every landlord ID-verified" — the Landlord role is only granted on verification approval). The reference mock said "2,400+ listings"; on a page rendering 13 that is the one claim a visitor can disprove instantly. **Don't put aspirational numbers in this strip.**
- **Map/list balance:** the map was a fixed `875px`, which at 1440 left the list ~490px to fit a 3-column grid — roughly 155px a card, so every title truncated. Now `lg:grid-cols-[1fr_420px] xl:grid-cols-[1fr_560px]` with a 2-column card grid. Size the grid so a card is never narrower than its own title.

## 6j. Public header — nav actions + account dropdown (redesigned July 24 2026)
The sticky public header (`layouts/app.blade.php`, `z-[100]`, see §5) carries the logo, primary nav (Browse / Areas / How it works), the scroll-collapse search pill, and the right-side authenticated actions. The July 24 pass was a minimal refresh *within* the locked palette — a refinement, not a new identity.

- **Border discipline on the right side.** The three actions used to be three bordered pills competing (bell, CTA, avatar). Now: the **notification bell is a borderless ghost** icon (`text-[#64748B] hover:bg-[#F7FCFC]`, and `bg-[#EEF8F8] text-[#156F8C]` when its menu is open) matching the nav links; the CTA (**Become a Landlord / Landlord Dashboard / Admin Actions**) is a **soft mist-teal pill** (`bg-[#EEF8F8] text-[#156F8C] hover:brightness-95`), not bordered white. Only the avatar chip and the one CTA carry weight.
- **Unread dot is teal `#2AA7A1`, not coral.** Coral is CTA-only (§3) — a notification dot is status, so it stays teal-family. Don't "brighten" it to coral.
- **Account dropdown is an identity card, not a flat list.** A mist gradient header band (`bg-gradient-to-br from-[#EEF8F8] to-[#F7FCFC]`) with a ring-bordered avatar, name, a small teal **role pill**, and email — that band is the one distinctive touch. Every menu row shares one style: `rounded-xl`, mist hover (`hover:bg-[#EEF8F8]`, **not** the old grey `#E2E8F0/60`), and the icon shifts muted `#64748B` → primary `#156F8C` on row hover (`group-hover:`). Sign out gets a red-tint hover (`#EF4444/[0.07]`). Floating elevation is `shadow-[0_16px_48px_-12px_rgba(15,23,42,0.20)]` + `ring-1 ring-[#E2E8F0]`.
- **All three header dropdowns are Alpine** (`x-data="{ open }"` + `@click.outside` + `@keydown.escape.window`), matching the Areas and notification menus, with the app's standard scale/fade/translate transition and a chevron that rotates on open. The avatar and Admin menus used to be a **bespoke ~50-line plain-JS `classList` toggle** — that handler was deleted; don't reintroduce a hand-rolled toggle (it can't animate without the rAF dance in §Modals of RULES.md, which is why the redesign moved to Alpine instead).
...
**Vacancy Watch answers that instead.** It leads with **rent not being earned** — the summed monthly `rental_fee` of every Available unit, in pesos — then lists the five longest-empty units with a days-vacant pill, the monthly rent each is losing, and a link straight to that unit's edit page. Every element states a cost and offers a next step.

- **Days vacant counts from `vacated_at`, falling back to `created_at`.** A unit that has *never* been let has no `vacated_at`; treating that as "no data" would hide the worst cases, because a listing nobody has ever taken is a longer and more urgent vacancy than a tenancy that ended last week — not an absent one. A footnote states the rule, since a 200-day number is alarming without it.
- **Escalation is by tint only, at 30 and 60 days** (`#F7FCFC` → amber `#FBBF24` → red `#EF4444` families) — never by size or icon, matching the deadline-banner rule in §7. **60 days is the same threshold `Admin\UnitCatalogueController` uses for "stale"**, so one number defines a long vacancy across the app rather than each page inventing its own.
- **The empty state is a win, not a blank.** Zero vacant units renders a green check and "Every unit is reserved, occupied, or under maintenance" — the one time this panel has good news, it should say so.
...
- Minimum contrast ratio: 4.5:1 for normal text (why `#DA8E77` is banned as small foreground text on a light ground — use `#A8573F` there, see §3)
...
- [ ] No `#DA8E77` as small foreground text on a light ground — `#A8573F` there instead (§3)
...
- Empty states: icon in an `bg-[#ECEEF6]` circle with `text-[#A8573F]` icon (not gray), bold title + muted subtitle.
...
## 13. `<x-styled-select>` — every dropdown, no more native `<select>` (completed July 25, 2026)
Native `<select>` can be styled up to its closed trigger, but the open options list is rendered by the OS and ignores CSS entirely — the "plain browser dropdown" look that stuck out against everything else in §0–§12. `resources/views/components/styled-select.blade.php` is an Alpine-driven replacement that now covers every plain filter/sort/form dropdown app-wide (admin, landlord, tenant, reports, profile, the hero search pill). Read the component's own doc comment for the full prop contract before touching it; the short version:
- `name` + `options` (`value => label` map) + `selected` — posts a hidden input like a native select, for GET-filter and POST-form dropdowns.
- `autosubmit="true"` — replaces `onchange="this.form.submit()"` / `onchange="document.getElementById('x').submit()"`.
- `required` — HTML's `required` is a no-op on the component's hidden `type="hidden"` input (browsers exempt hidden inputs from constraint validation), so the component validates on the closest form's submit instead and flags the trigger red + opens the panel. Only add this prop where the original `<select>` had a literal `required` attribute — a red asterisk in the label text is not the same thing and must not trigger it.
- `x-model="var"` — for dropdowns living inside an existing Alpine form scope (`x-modelable="value"` under the hood), e.g. the unit-type/capacity selects in `landlord/units/create.blade.php`.
...
- **New palette:** `#060D26` deep navy (was `#156F8C`/`#1F2937`/`#FF8A65`/`#0F172A` — four old roles
  collapsed into one), `#DA8E77` gold (was `#69D2C6`, and `#2AA7A1` where it wasn't a solid fill),
  `#A8573F` dark gold (the new text-safe accent, no prior equivalent — see §3), `#5B6A8E` slate,
...
- **`#2AA7A1` needed judgment, not a find/replace, and was the single biggest risk in the job.** It
  ...
  rings, focus states, tinted backgrounds, active-state accents — became `#DA8E77` gold; small
  text-sized foreground uses (labels, icons, checkbox fills) became `#A8573F` dark gold specifically
...
- **New selected-unit chip** (`x-show="selected"`, so it's absent until a unit is picked): a
  `bg-[#DA8E77]/10 border-[#DA8E77]/30` pill with the same checkmark glyph, `Unit: {{ selected.label
